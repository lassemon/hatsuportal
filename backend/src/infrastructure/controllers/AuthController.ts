...
import { IUserApplicationMapper } from '@hatsuportal/user-management'
...
export class AuthController extends RootController {
  ...
  constructor() {
    ...
    this.userApplicationMapper = tsyringeContainer.resolve<IUserApplicationMapper>('IUserApplicationMapper')
    this.authApiMapper = tsyringeContainer.resolve<IAuthApiMapper>('IAuthApiMapper')
  }
  ...
  public async status(
    ...
    try {
      ...
      statusResponse(200, this.userApiMapper.toResponse(request.user))
    } catch (error) {
      const httpError = this.httpErrorMapper.mapApplicationErrorToHttpError(error)
      errorResponse(httpError.status, httpError)
    }

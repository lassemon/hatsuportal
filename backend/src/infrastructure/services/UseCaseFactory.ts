...
import { ErrorHandlingUseCaseDecorator, ITransactionManager } from '@hatsuportal/foundation'
...
import { StoryWriteRepositoryWithCache } from '../repositories/StoryWriteRepositoryWithCache'
import { ImageRepositoryWithCache } from '../repositories/ImageRepositoryWithCache'
import { StoryReadRepositoryWithCache } from '../repositories/StoryReadRepositoryWithCache'
import { CommentWriteRepositoryWithCache } from '../repositories/CommentWriteRepositoryWithCache'
export class UseCaseFactory implements IPostUseCaseFactory, IUserUseCaseFactory, IMediaUseCaseFactory {
  ...
  constructor(
    ...
    private readonly commentWriteRepository: ICommentWriteRepository,
    ...
    const mediaCommandFacade = new MediaCommandFacade(
      // THESE USE CASES ARE DEPENDENT ON userGatewayForMediaManagement, MUST INITIALIZE GATEWAU BEFORE USE CASES
  ...
  createLoginUserUseCase(): ILoginUserUseCase {
    return new LoginUserUseCaseWithValidation(new LoginUserUseCase(this.userApplicationMapper, this.userRepository, this.tokenService))
  }

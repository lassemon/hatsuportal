...
import { Base64Image, FileName, InvalidBase64ImageError, InvalidInputError, MimeType } from '@hatsuportal/common-bounded-context'
...
describe('ImageService', () => {
  ...
  it('uses image processing service to resize image buffer', async ({ unitFixture }) => {
    ...
    const imageService = new ImageFileService(imageProcessingServiceMock, imageStorageServiceMock)
    await imageService.resizeImageBuffer(unitFixture.base64ImageBufferMock())
  ...
  it('uses image processing service to validate mime type', async ({ unitFixture }) => {
    ...
    await imageService.validateMimeType(unitFixture.base64ImageBufferMock(), new FileName(unitFixture.imageDTOMock().fileName))
    expect(spy).toBeCalledTimes(1)
  })
  ...
  it('fails to validate mime type', async ({ unitFixture }) => {
    ...
    const imageService = new ImageFileService(imageProcessingServiceMock, imageStorageServiceMock)
    await expect(
      imageService.validateMimeType(unitFixture.base64ImageBufferMock(), new FileName(unitFixture.imageDTOMock().fileName))
  ...
  it('calls image storage service to write file to file system', ({ unitFixture }) => {
    ...
    const imageService = new ImageFileService(imageProcessingServiceMock, imageStorageServiceMock)
    imageService.writeImageToFileSystem(unitFixture.base64ImageBufferMock(), new FileName(unitFixture.imageDTOMock().fileName))
    expect(spy).toBeCalledTimes(1)
  })

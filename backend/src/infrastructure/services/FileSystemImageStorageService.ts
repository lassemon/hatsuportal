...
export class FileSystemImageStorageService implements IImageStorageService {
  ...
  async storeImageBuffer(imageBuffer: Buffer, storageKey: NonEmptyString): Promise<void> {
    const outputPath = this.resolveSafeImagePath(storageKey)
    try {
  ...
  async getImage(storageKey: NonEmptyString): Promise<string> {
    try {
      const imagePath = this.resolveSafeImagePath(storageKey)
  ...
  async copyImage(sourceStorageKey: NonEmptyString, destinationStorageKey: NonEmptyString): Promise<void> {
    const sourcePath = this.resolveSafeImagePath(sourceStorageKey)
    const destinationPath = this.resolveSafeImagePath(destinationStorageKey)
    try {
  ...
  async deleteImage(storageKey: NonEmptyString) {
    const unlinkPath = this.resolveSafeImagePath(storageKey)
    try {
  ...
  private resolveSafeImagePath(storageKey: NonEmptyString): string {
    const sanitizedStorageKey = sanitize(storageKey.value)
    const basePath = path.resolve(this.imagesBasePath)
    const resolvedPath = path.resolve(basePath, sanitizedStorageKey)
    if (!resolvedPath.startsWith(basePath + path.sep) && resolvedPath !== basePath) {
      throw new DataPersistenceError({ message: `Invalid storage key ${storageKey.value}` })
    }
    return resolvedPath
  }
}

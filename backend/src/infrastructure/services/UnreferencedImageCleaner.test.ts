...
import { IUserReadRepository, SystemUserId } from '@hatsuportal/user-management'
...
describe('UnreferencedImageCleaner', () => {
  const setup = () => {
    ...
    const userReadRepository = {
      findAllReferencedProfileImageIds: vi.fn().mockResolvedValue(['image-c'])
    ...
    return { imageRepository, storyReadRepository, userReadRepository, deleteImageUseCase, cleanupLock, cleaner }
  }

  it('deletes images that are old enough and not referenced by story cover or profile links', async () => {
    ...
    expect(execute).toHaveBeenCalledWith(
      expect.objectContaining({
        deletedById: new SystemUserId().value,

 ...
 * on any story (`post_image_links`) or as a profile picture on any user (`user_image_links`).
 *
 * Typical causes:
...
export class UnreferencedImageCleaner {
  ...
  /** Deletes Media images that no story cover or user profile link references. */
  async cleanUnreferencedImages(): Promise<void> {
    ...
    try {
      const referencedImageIds = new Set([
        ...(await this.storyReadRepository.findAllReferencedCoverImageIds()),
        ...(await this.userReadRepository.findAllReferencedProfileImageIds())
      ])

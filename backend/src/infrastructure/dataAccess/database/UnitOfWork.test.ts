...
import { NodeAsyncLocalTransactionContext } from '@hatsuportal/platform'
...
describe('UnitOfWork', () => {
  it('deduplicates event holders returned from nested and parent callbacks', async () => {
    ...
    expect(domainEventService.persistEvents).toHaveBeenCalledWith([])
  ...
  it('passes through ApplicationError thrown inside the work callback', async () => {
    const transactionContext = new NodeAsyncLocalTransactionContext()
    const unitOfWork = new UnitOfWork(
      { persistEvents: vi.fn() } as never,
      {

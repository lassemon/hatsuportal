...
export class TransactionManager implements ITransactionManager {
  constructor(private readonly connectionProvider: IDatabaseConnectionProvider) {}

  async execute<T>(operation: () => Promise<T>, repositories: IRepository[] = []): Promise<T> {
    const database = await this.connectionProvider.getConnection()

    return await database.transaction(async (knexTransaction) => {

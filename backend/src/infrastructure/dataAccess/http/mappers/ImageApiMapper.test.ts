...
describe('ImageApiMapper', () => {
  ...
  it('converts create request to create input dto', ({ unitFixture }) => {
    ...
    const dto = imageMapper.toCreateImageInputDTO(createRequest, unitFixture.imageDTOMock().id)

    expect(dto.loggedInUserId).toBe(unitFixture.imageDTOMock().id)
  ...
  it('converts update request to update input dto', ({ unitFixture }) => {
    ...
    const dto = imageMapper.toUpdateImageInputDTO(updateRequest, unitFixture.imageDTOMock().id)

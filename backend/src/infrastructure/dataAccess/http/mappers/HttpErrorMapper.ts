...
import { DevelopmentPasswordPolicy, InvalidPasswordError } from '@hatsuportal/user-management'
import {
  ApplicationError,
  AuthenticationError,
  AuthorizationError,
        ...
        case InvalidPasswordError:
          return {
            ...
            message: `Given password is not valid. ${
              process.env.NODE_ENV === 'dev' ? new DevelopmentPasswordPolicy().getRulesMessage() : `` // do not expose password rules to the client in production environment
            }`
          }
        default:

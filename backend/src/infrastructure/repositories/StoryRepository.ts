...
export class StoryRepository extends Repository implements IStoryRepository {
  ...
  async countAll(creatorId?: string): Promise<number> {
    const database = await this.databaseOrTransaction()
    const result = await database(this.tableName)
      .whereNotIn('visibility', creatorId ? [VisibilityEnum.Private] : [VisibilityEnum.Private, VisibilityEnum.LoggedIn])
  ...
  async delete(storyId: PostId) {
    try {
      ...
      const database = await this.databaseOrTransaction()
      await database(this.tableName).where('id', storyId.value).delete()

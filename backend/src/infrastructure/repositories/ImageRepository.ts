...
export class ImageRepository extends Repository implements IImageRepository {
  ...
  async update(image: Image): Promise<Image> {
    try {
      ...
      updatedImage.update({ base64: this.imageFileService.convertBufferToBase64Image(resizedBuffer, image.mimeType) })
      return updatedImage
    } catch (error) {
      return this.throwDataPersistenceError(error)
    }
  ...
  async delete(image: Image) {
    try {
      ...
      if (affected === 0) throw new ConcurrencyError(`Image deletion failed because image '${image.id.value}' was deleted by another user.`)
    } catch (error: unknown) {
      return this.throwDataPersistenceError(error)
    }
  }
  ...
  private async validateAndPrepareImage(image: Image, buffer: Buffer): Promise<Image> {
    image.update({ mimeType: await this.imageFileService.validateMimeType(buffer, image.fileName) })
    return image
  }
  ...
  private async getPersistedImage(image: Image, buffer: Buffer): Promise<Image> {
    ...
    insertedImage.update({ base64: this.imageFileService.convertBufferToBase64Image(buffer, image.mimeType) })

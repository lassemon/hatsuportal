...
import { IDomainEvent } from '@hatsuportal/common-bounded-context'
...
> {
  ...
  thenDomainEventsEmitted(...eventKeys: (keyof DOMAIN_EVENTS)[]) {
    ...
    eventKeys.forEach((key) =>
      expect(this.transactionBundle.domainEventDispatcherMock.dispatch).toHaveBeenCalledWith(expect.any(this.domainEvents[key]))
    )
    return this
  }
  ...
  thenDomainEventsNotEmitted(...eventKeys: (keyof DOMAIN_EVENTS)[]) {
    ...
    this.ensureNoUnexpectedError()
    eventKeys.forEach((key) =>
      expect(this.transactionBundle.domainEventDispatcherMock.dispatch).not.toHaveBeenCalledWith(expect.any(this.domainEvents[key]))
    )
    return this
  }

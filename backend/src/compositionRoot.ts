...
import { NodeAsyncLocalTransactionContext } from './infrastructure/dataAccess/database/NodeAsyncLocalTransactionContext'
...
interface ServiceInstances {
  ...
  unitOfWork: IUnitOfWork<UniqueId, UnixTimestamp>
...
function createRepositories(
  ...
  const userRepository = new UserRepositoryWithCache(
    new UserRepository(dataAccessProvider, helpers, transactionContext, mappers.userInfrastructure),
...
function createInstances(): Instances {
  ...
  const repositories = createRepositories(dataAccess.dataAccessProvider, dataAccess.helpers, mappers, transactionContext)
  ...
  const systemUserId = new SystemUserId(config.systemUser.id)

...
function createServices(
  ...
  const imageStorageService = process.env.NODE_ENV === 'prod' ? new TigrisImageStorageService() : new FileSystemImageStorageService()
...
function createInstances(): Instances {
  ...
  const cronJobs = createCronJobs(repositories.domainEvent, mappers, dataAccess.eventDispatcher, repositories, services)

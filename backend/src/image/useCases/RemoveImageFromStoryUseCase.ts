...
import { PostId, IImageMetadataRepository, IStoryRepository, Story } from '@hatsuportal/domain'
...
export class RemoveImageFromStoryUseCase implements IRemoveImageFromStoryUseCase {
  ...
  async execute({ removeImageFromStoryInput, imageRemoved }: IRemoveImageFromStoryUseCaseOptions): Promise<void> {
    try {
      ...
      if (existingStory?.imageId) {
        const existingImage = await this.imageMetadataRepository.findById(existingStory.imageId)
        if (existingImage) {
      ...
      const storyWithoutImage = new Story({ ...this.storyApplicationMapper.toDTO(existingStory), imageId: null })

...
export interface Configs {
  ...
  cache: {
    repositoryTTLSeconds: number
    mediaStorageTTLSeconds: number
  }
  systemUser: {
    id: string
    name: string
...
const defaultConfigs: Configs = {
  ...
  images: {
    ...
    orphanMinAgeMs: 10 * 60 * 1000, // 10 minutes
  ...
  cache: {
    repositoryTTLSeconds: 4, // 4 seconds protect from multiple queries during same operation, but we do not want to offer stale data
    mediaStorageTTLSeconds: 60
  },
  systemUser: {
    id: process.env.SYSTEM_USER_ID ?? SystemUserId.DEFAULT,

...
import { IStoryRepository, PostId, UserId } from '@hatsuportal/domain'
...
export class FindStoryUseCase implements IFindStoryUseCase {
  ...
  async execute({ findStoryInput, storyFound }: IFindStoryUseCaseOptions): Promise<void> {
    ...
    try {
      ...
      if (loggedInUserId && story.visibility.equals(VisibilityEnum.Private) && !story.createdBy.equals(new UserId(loggedInUserId)))

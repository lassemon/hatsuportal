...
import { NotFoundError, UseCaseWithValidation } from '@hatsuportal/common-bounded-context'
...
export class FindStoryUseCaseWithValidation extends UseCaseWithValidation<IFindStoryUseCaseOptions> implements IFindStoryUseCase {
  ...
  async execute(options: IFindStoryUseCaseOptions): Promise<void> {
    ...
    const valid = (await this.validateAuthorization(story, loggedInUserId)) && this.validateDomainRules(options)
  ...
  async validateAuthorization(story: Story, loggedInUserId?: string): Promise<boolean> {
    ...
    if (!authorizationResult.isAuthorized) throw new NotFoundError(authorizationResult.reason)

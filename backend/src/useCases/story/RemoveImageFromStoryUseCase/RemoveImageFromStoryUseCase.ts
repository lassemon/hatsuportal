...
import { IStoryRepository, PostId, Story } from '@hatsuportal/post-management'
...
export class RemoveImageFromStoryUseCase implements IRemoveImageFromStoryUseCase {
  ...
  async execute({ removeImageFromStoryInput, imageRemoved }: IRemoveImageFromStoryUseCaseOptions): Promise<void> {
    try {
      ...
      if (!existingStory.createdById.equals(loggedInUserId)) {
      ...
      const storyWithoutImage = Story.reconstruct({ ...this.storyApplicationMapper.toDTO(existingStory), image: null })

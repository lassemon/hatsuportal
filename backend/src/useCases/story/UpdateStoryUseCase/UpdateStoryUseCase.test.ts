...
describe('UpdateStoryUseCase', () => {
  ...
  it('should call updateConflict callback and throw when ConcurrencyError occurs', async ({ unitFixture }) => {
    const scenario = await UpdateStoryScenario.given()
      .withLoggedInUser()
      .withExistingStory()
      .repositoryWillReject('update', new ConcurrencyError('conflict', unitFixture.storyDTOMock()))
      .expectErrorOfType(ConcurrencyError)
      .whenExecutedWithInput({
        loggedInUserId: unitFixture.userDTOMock().id,
        updateStoryData: {
          id: unitFixture.storyDTOMock().id,
  ...
  it('should rollback transaction when story repository fails', async ({ unitFixture }) => {
    const scenario = await UpdateStoryScenario.given()
      .withLoggedInUser()
      .withExistingStory()
      .repositoryWillReject('update', new Error('Database error'))

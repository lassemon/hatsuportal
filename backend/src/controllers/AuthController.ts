...
import { UserApplicationMapper, UserDTO } from '@hatsuportal/user-management'
import { RootController } from './RootController'
import { ErrorPresentationMapper, ErrorResponse, TsoaRequest } from '@hatsuportal/presentation-common'
import { LoginRequest, UserResponse, UserPresentationMapper, AuthPresentationMapper } from '@hatsuportal/presentation-user'
...
export class AuthController extends RootController {
  ...
  protected readonly authPresentationMapper: AuthPresentationMapper
  ...
  constructor() {
    ...
    this.authPresentationMapper = container.resolve('IAuthPresentationMapper')
  ...
  public async login(
    ...
    try {
      const loginUserInput = this.authPresentationMapper.toLoginUserInputDTO(loginRequest)
      ...
      await loginUserUseCase.execute({
        ...
        loginSuccess: (authToken, refreshToken, user) => {
          ...
          loginResponse(200, this.userPresentationMapper.toResponse(user), {
            'Set-Cookie': [
  ...
  public async status(
    ...
    try {
      ...
      statusResponse(200, this.userPresentationMapper.toResponse(this.userApplicationMapper.toDTO(request.user)))
    } catch (error) {
      const httpError = this.errorPresentationMapper.mapApplicationErrorToHttpError(error)
      errorResponse(httpError.status, httpError)
    }

...
export class UserPresentation {
  ...
  constructor(props: UserPresentationDTO) {
    if (isNonStringOrEmpty(props.id)) {
      throw new InvalidPresentationPropertyError(`Property "id" must be a string, was '${props.id}'`)
    }
    this._id = props.id
    ...
    if (!isString(props.email)) {
      throw new InvalidPresentationPropertyError(`Property "email" must be a string, was '${props.email}'`)
    }
    this._email = props.email

    if (!props.roles || props.roles.length <= 0) throw new InvalidPresentationPropertyError('User must have at least one role.')
  ...
  set email(email: string) {
    if (!isString(email)) {
      throw new InvalidPresentationPropertyError(`Property "email" must be a string, was '${email}'`)
    }
    this._email = email
  }
  ...
  set roles(roles: UserRoleEnum[]) {
    if (!roles || roles.length <= 0) throw new InvalidPresentationPropertyError('User must have at least one role.')
    this._roles = roles
  }
  ...
  set updatedAt(updatedAt: number | null) {
    if (!isNumber(updatedAt) && updatedAt !== null) {
      throw new InvalidPresentationPropertyError(`Property "updatedAt" must be a number or null, was '${updatedAt}'`)
    }

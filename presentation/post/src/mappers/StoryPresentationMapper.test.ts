...
import { ImagePresentationMapper } from '@hatsuportal/presentation-common'
...
describe('StoryPresentationMapper', () => {
  ...
  it('converts story response to StoryPresentation entity', ({ unitFixture }) => {
    expect(storyMapper.toStoryPresentation(unitFixture.storyDTOMock())).toBeInstanceOf(StoryPresentation)
    expect(storyMapper.toStoryPresentation(unitFixture.storyDTOMock()).image).toBeInstanceOf(ImagePresentation)
  })
})

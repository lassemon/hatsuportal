...
export class StoryReadRepositoryWithCache implements IStoryReadRepository {
  constructor(
    private readonly baseRepo: IStoryReadRepository,
    private readonly cache: ICache<StoryReadModelDTO>
  ) {}

  async findAll(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryReadModelDTO[]> {
    return await this.baseRepo.findAll(orderBy, order)
  }
  ...
  async findAllReferencedCoverImageIds(): Promise<string[]> {
    return await this.baseRepo.findAllReferencedCoverImageIds()
  }
  ...
  invalidateById(storyId: PostId): void {
    this.cache.delete(`findById:${storyId.value}`)
  }
}

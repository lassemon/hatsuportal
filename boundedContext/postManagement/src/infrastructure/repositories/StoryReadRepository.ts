import { SortableKeyEnum, OrderEnum, VisibilityEnum, isEnumValue, ImageRoleEnum } from '@hatsuportal/common'
import { RepositoryBase, IDataAccessProvider, IRepositoryHelpers, ITransactionContext, IQueryBuilder } from '@hatsuportal/platform'
...
export class StoryReadRepository extends RepositoryBase implements IStoryReadRepository {
  private readonly imageLinkTable = 'post_image_links'
  ...
  constructor(
    ...
    super(dataAccessProvider, helpers, transactionContext, 'story_enriched_read_view')
  }

  async findAll(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryReadModelDTO[]> {
    const stories = await this.table<StoryReadDatabaseSchema>().select(`*`).modify(withOrderBy(orderBy, order))
    return stories.map(this.storyMapper.toDTO)
  }

  async findAllPublic(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryReadModelDTO[]> {
  ...
  async findLatest(limit: number, loggedIn: boolean): Promise<StoryReadModelDTO[]> {
    const stories = await this.table<StoryReadDatabaseSchema>()
      ...
      .modify(withOrderBy(SortableKeyEnum.CREATED_AT, OrderEnum.Descending))
      .limit(limit || 5)
  ...
  async findByImageId(imageId: PostId): Promise<StoryReadModelDTO[]> {
    ...
    const links = await database
      .table<Pick<PostImageLinkDatabaseSchema, 'postId'>>(this.imageLinkTable)
      .select('postId')
      .where({ imageId: imageId.value, role: ImageRoleEnum.Cover })

    if (links.length === 0) {
      return []
    }

    return this.findByIds(links.map((link) => new PostId(link.postId)))
  }

  async findAllReferencedCoverImageIds(): Promise<string[]> {
    const database = await this.database()
    const rows = await database.table<Pick<PostImageLinkDatabaseSchema, 'imageId'>>(this.imageLinkTable).distinct().select('imageId')
  ...
  async findAllForCreator(creatorId: PostCreatorId): Promise<StoryReadModelDTO[]> {
    const stories = await this.table<StoryReadDatabaseSchema>()
      ...
      .modify(withOrderBy(SortableKeyEnum.TITLE, OrderEnum.Ascending))
  ...
  async findByIds(ids: PostId[]): Promise<StoryReadModelDTO[]> {
    if (ids.length === 0) {
      return []
    }
    const stories = await this.table<StoryReadDatabaseSchema>()
  ...
  invalidateById(_storyId: PostId): void {
    // no-op: uncached read repository has nothing to invalidate
    // this is here only to consolidate the IStoryReadRepository interface
  }
}
...
const withOrderBy = (orderBy: SortableKeyEnum, order: OrderEnum) => (queryBuilder: IQueryBuilder<StoryReadDatabaseSchema>) => {
  if (orderBy) {
    const _orderBy = isEnumValue(orderBy, SortableKeyEnum) ? orderBy : SortableKeyEnum.TITLE
    const _order = order === OrderEnum.Ascending || order === OrderEnum.Descending ? order : OrderEnum.Descending
    switch (orderBy) {
      case SortableKeyEnum.CREATED_BY:
        queryBuilder.orderBy(`createdByName`, _order)
        break
      case SortableKeyEnum.TITLE:
      case SortableKeyEnum.VISIBILITY:
...
const withWordSearch = (wordSearch?: string) => (queryBuilder: IQueryBuilder<StoryReadDatabaseSchema>) => {
  if (wordSearch) {
    queryBuilder.andWhere(function (_queryBuilder) {
      _queryBuilder.whereILike('title', `%${wordSearch}%`)
      _queryBuilder.orWhereILike('body', `%${wordSearch}%`)
    })
  }
}

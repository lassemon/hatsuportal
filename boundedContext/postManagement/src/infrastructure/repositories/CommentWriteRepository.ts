...
export class CommentWriteRepository extends RepositoryBase implements ICommentWriteRepository {
  constructor(
    ...
    super(dataAccessProvider, helpers, transactionContext, 'comments')
  }

  async findByIdForUpdate(id: CommentId): Promise<Comment | null> {
    const comment = await this.table<CommentDatabaseSchema>().where('id', id.value).forUpdate().first()
    if (!comment) {
      return null
    }

    this.registerExpectedUpdatedAt(comment.id, new UnixTimestamp(comment.updatedAt || unixtimeNow()))
  ...
  async update(comment: Comment): Promise<Comment> {
    const expectedUpdatedAt = this.requireExpectedUpdatedAt(comment.id.value)
    const commentToUpdate = this.commentMapper.toCommentUpdateRecord(comment)
    const affected = await this.table<CommentDatabaseSchema>()
      .where({ id: comment.id.value, updatedAt: expectedUpdatedAt.value })
      .update({ ...commentToUpdate, updatedAt: unixtimeNow() })
    if (affected.length === 0) throw new ConcurrencyError<Comment>(`Comment ${comment.id} was modified by another user`, comment)

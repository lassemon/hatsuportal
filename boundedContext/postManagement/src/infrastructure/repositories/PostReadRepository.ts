...
import { IDataAccessProvider, IQueryBuilder, IRepositoryHelpers, ITransactionContext, RepositoryBase } from '@hatsuportal/platform'
...
export class PostReadRepository extends RepositoryBase implements IPostReadRepository {
  constructor(
  ...
  async findById(id: PostId): Promise<PostDTO | null> {
    const post = await this.table<PostDatabaseSchema>().where({ id: id.value }).first()

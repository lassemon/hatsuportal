...
export class StoryWriteRepository extends RepositoryBase implements IStoryWriteRepository {
  private readonly imageLinkTable = 'post_image_links'
  private readonly tagLinkTable = 'post_tag_links'
  private readonly commentsTable = 'comments'
  ...
  constructor(
    ...
    super(dataAccessProvider, helpers, transactionContext, 'stories')
  }
  ...
  async findByIdForUpdate(storyId: PostId): Promise<Story | null> {
    ...
    this.registerExpectedUpdatedAt(story.id, new UnixTimestamp(post.updatedAt || unixtimeNow()))
  ...
  async update(story: Story) {
    // expectedUpdatedAt is set in findByIdForUpdate
    // this forces use cases to load the story before updating it.
    // protects against race conditions
    const expectedUpdatedAt = this.requireExpectedUpdatedAt(story.id.value)

    // 1) Update the supertype row
    const postUpdate = this.postMapper.toPostUpdateRecord(story, EntityTypeEnum.Story)
    const postAffected = await this.postWriteRepository.update(postUpdate, expectedUpdatedAt)

    // Optimistic locking pattern
    if (postAffected.length === 0) {
  ...
  async delete(story: Story) {
    try {
      ...
      // Optimistic locking pattern
      const affected = await this.postWriteRepository.deleteById(story.id.value, expectedUpdatedAt)
  ...
  private async findStoryByIdRAW(storyId: string): Promise<StoryDatabaseSchema | null> {
    ...
    const story = await this.table<StoryDatabaseSchema>()
      .leftJoin('posts', (join) => join.on('posts.id', 'stories.id'))
      ...
      .joinRaw(
        `
          LEFT JOIN LATERAL (
            SELECT array_remove(array_agg(${this.commentsTable}.id ORDER BY ${this.commentsTable}.created_at), NULL) AS comment_ids
            FROM ${this.commentsTable}
            WHERE ${this.commentsTable}.post_id = posts.id
          ) AS post_comment_aggregation ON TRUE
        `
      )
      .select(
        `${this.tableName}.*`,
        ...
        database.raw('COALESCE(post_comment_aggregation.comment_ids, ARRAY[]::uuid[]) AS comment_ids')
      )

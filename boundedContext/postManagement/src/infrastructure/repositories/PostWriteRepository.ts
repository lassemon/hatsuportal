...
export class PostWriteRepository extends RepositoryBase implements IPostWriteRepository {
  ...
  async update(post: PartialExceptFor<PostDatabaseSchema, 'id'>, baseline: UnixTimestamp): Promise<PostDatabaseSchema[]> {
    return await this.table<PostDatabaseSchema>().where({ id: post.id, updatedAt: baseline.value }).update(post)
  }

...
describe('MediaGateway', () => {
  ...
  it('delegates staging workflow operations to media command facade', async () => {
    const { mediaCommandFacade, gateway } = setup()
    const createCommand: mediaV1.CreateStagedImageCommand = {
      ownerEntityType: EntityTypeEnum.Story,
      ownerEntityId: Fixture.sampleStoryId,
    ...
    const prepared: mediaV1.PreparedStagedImageContract = {
      imageId: Fixture.sampleImageId,
      stagedVersionId: 'staged-version-id',
    ...
    vi.mocked(mediaCommandFacade.prepareStagedImageFile).mockResolvedValue(prepared)

    await expect(gateway.prepareStagedImageFile(createCommand)).resolves.toEqual(prepared)
    await expect(gateway.registerPreparedStagedImageFileRollbackCleanup(prepared)).resolves.toBeUndefined()
    await expect(gateway.saveStagedImageMetadata(prepared)).resolves.toBeUndefined()

    expect(mediaCommandFacade.prepareStagedImageFile).toHaveBeenCalledWith(createCommand)
    expect(mediaCommandFacade.registerPreparedStagedImageFileRollbackCleanup).toHaveBeenCalledWith(prepared)
    expect(mediaCommandFacade.saveStagedImageMetadata).toHaveBeenCalledWith(prepared)
  })
})

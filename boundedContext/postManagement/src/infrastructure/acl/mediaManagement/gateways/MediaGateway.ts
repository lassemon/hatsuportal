  ...
  async getImageById(params: { imageId: string }): Promise<ImageLoadResult<ImageAttachmentReadModelDTO, ImageLoadError>> {
    try {
      ...
      if (error instanceof Error) {
        return ImageLoadResult.failedToLoad(params.imageId, error)
      } else {
        return ImageLoadResult.failedToLoad(params.imageId, new ApplicationError({ message: 'Unknown error occurred', cause: error }))
      }
    }
  }

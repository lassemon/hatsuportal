...
export class MediaGateway implements IMediaGateway {
  ...
  async getImageById(params: { imageId: string }): Promise<EntityLoadResult<ImageAttachmentReadModelDTO, EntityLoadError>> {
    try {
      ...
      if (error instanceof Error) {
        return EntityLoadResult.failure(new EntityLoadError({ entityId: params.imageId, error }))
      } else {

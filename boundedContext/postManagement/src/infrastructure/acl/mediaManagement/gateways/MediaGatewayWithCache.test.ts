...
import { EntityLoadError, EntityLoadResult, MapCache } from '@hatsuportal/platform'
...
describe('MediaGatewayWithCache', () => {
  ...
  it('does not cache failed lookups', async () => {
    ...
    const failed = EntityLoadResult.failure(new EntityLoadError({ entityId: Fixture.sampleImageId, error: new Error('not found') }))

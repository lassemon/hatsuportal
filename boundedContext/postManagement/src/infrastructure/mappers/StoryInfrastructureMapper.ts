import { PostCreatorId, PostId, PostTitle, PostVisibility, Story, StoryBody, TagId } from '../../domain'
...
export interface IStoryInfrastructureMapper {
  toStoryInsertRecord(
    ...
    StoryDatabaseSchema,
    'title' | 'visibility' | 'postType' | 'coverImageId' | 'tagIds' | 'commentIds' | 'createdById' | 'createdAt' | 'updatedAt'
  >
  toStoryUpdateRecord(
    story: Story
  ): PartialExceptFor<Omit<StoryDatabaseSchema, 'title' | 'visibility' | 'postType' | 'coverImageId' | 'tagIds' | 'commentIds'>, 'id'>
...
export class StoryInfrastructureMapper implements IStoryInfrastructureMapper {
  ...
  toImageLinkRow(story: Story): Omit<PostImageLinkDatabaseSchema, 'createdAt'> | null {
    const hasCoverImage = story.coverImageId && !story.coverImageId.equals(CoverImageId.NOT_SET)
  ...
  toDomainEntity(storyRecord: StoryDatabaseSchema): Story {
    return Story.reconstruct({
      ...
      title: new PostTitle(storyRecord.title),
      body: new StoryBody(storyRecord.body),
      coverImageId: storyRecord.coverImageId ? new CoverImageId(storyRecord.coverImageId) : CoverImageId.NOT_SET,
      tagIds: storyRecord.tagIds.map((id) => new TagId(id)),
      createdAt: new CreatedAtTimestamp(storyRecord.createdAt),

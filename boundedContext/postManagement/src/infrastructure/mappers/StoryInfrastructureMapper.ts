...
import { IImageApplicationMapper, ImageId, ImageLoadErrorDTO, Image } from '@hatsuportal/common-bounded-context'
import { StoryDTO } from '../../application/dtos'
...
export class StoryInfrastructureMapper implements IStoryInfrastructureMapper {
  ...
  toDTO(storyRecord: StoryDatabaseSchema, image?: Image): StoryDTO {
    ...
    return {
      ...
      // TODO, it is smelly to use application mapper in infrastructure mapper
  ...
  toDomainEntity(storyRecord: StoryDatabaseSchema, image?: Image, imageLoadError?: Maybe<Error>): Story {
    const story = Story.reconstruct(this.toDTO(storyRecord, image))

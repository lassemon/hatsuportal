...
describe('PostInfrastructureMapper', () => {
  ...
  it('converts database schema to dto', ({ unitFixture }) => {
    const record = unitFixture.postDatabaseRecord()
    const dto = mapper.toDTO(record)

    expect(dto.id).toBe(record.id)
    expect(dto.visibility).toBe(VisibilityEnum.Public)
    expect(dto.postType).toBe(EntityTypeEnum.Story)

import { CreatedAtTimestamp, DomainEvent, Entity, EntityProps, UniqueId, UnixTimestamp } from '@hatsuportal/shared-kernel'
import { unixtimeNow } from '@hatsuportal/common'
import { CommentId } from '../valueObjects/CommentId'
import { CommentBody } from '../valueObjects/CommentBody'
...
export interface CommentProps extends EntityProps {
  ...
  body: CommentBody | null // null if isDeleted
...
export class Comment extends Entity {
  ...
  static assertCanCreate(props: any): void {
    new Comment(
      ...
      props.body === null ? null : props.body instanceof CommentBody ? props.body : new CommentBody(props.body),
      CommentId.fromOptional(props.parentCommentId),
      props.isDeleted,
  ...
  private _body: CommentBody | null
  ...
  public writeBody(body: CommentBody): void {
    this._body = body
  ...
  public get body(): CommentBody | null {
    return this._body
  }
  ...
  public softDelete(deleteById: UniqueId): void {
    if (!this._isDeleted) {
  ...
  public delete(deleteById: UniqueId): void {
    if (!this._isDeleted) {
      this._isDeleted = true
      this._updatedAt = new UnixTimestamp(unixtimeNow())
      this.addDomainEvent(

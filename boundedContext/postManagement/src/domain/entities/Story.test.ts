...
import { uuid, VisibilityEnum } from '@hatsuportal/foundation'
...
describe('Story', () => {
  ...
  it('does not allow creating an story without an id', ({ unitFixture }) => {
    ...
    expect(() => {
      Story.canCreate(postWithoutId as any, { throwError: true })
    }).toThrow(InvalidPostIdError)
  })

  it('does not allow creating an story with an id with empty spaces', ({ unitFixture }) => {
    expect(() => {
      Story.canCreate({ ...unitFixture.storyDTOMock(), id: ' te st ' } as any, { throwError: true })
    }).toThrow(InvalidPostIdError)
  })
  ...
  it('does not allow creating a story without a name', ({ unitFixture }) => {
    ...
    expect(() => {
      Story.canCreate(storyWithoutName as any, { throwError: true })
    }).toThrow()
  })
  ...
  it('does not allow creating a story without a description', ({ unitFixture }) => {
    ...
    expect(() => {
      Story.canCreate(storyWithoutDescription as any, { throwError: true })
    }).toThrow()
  })

import { describe, expect, it } from 'vitest'
import Recipe from './Recipe'
import { InvalidPostIdError } from '../errors/InvalidPostIdError'
import { uuid } from '@hatsuportal/common'
import { InputLimits } from '@hatsuportal/contracts'
import { CreatedAtTimestamp, UnixTimestamp } from '@hatsuportal/shared-kernel'
import { PostCreatorId } from '../valueObjects/PostCreatorId'
import { PostId } from '../valueObjects/PostId'
import { PostVisibility } from '../valueObjects/PostVisibility'
import { PostTitle } from '../valueObjects/PostTitle'
import { RecipeDescription } from '../valueObjects/RecipeDescription'
import { PostTitleTooLongError } from '../errors/PostTitleTooLongError'
import { RecipeDescriptionTooLongError } from '../errors/RecipeDescriptionTooLongError'

describe('Recipe', () => {
  it('can create recipe with all properties', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    const dto = unitFixture.recipeDTOMock()
    expect(recipe.id.value).toBe(dto.id)
    expect(recipe.createdById.value).toBe(dto.createdById)
    expect(recipe.createdAt.value).toBe(dto.createdAt)
    expect(recipe.updatedAt?.value).toBe(dto.updatedAt)
    expect(recipe.title.value).toBe(dto.title)
    expect(recipe.visibility.value).toBe(dto.visibility)
    expect(recipe.description.value).toBe(dto.description)
    expect(recipe.ingredients).toStrictEqual(dto.ingredients)
    expect(recipe.instructions).toStrictEqual(dto.instructions)
  })

  it('does not allow creating a recipe without an id', ({ unitFixture }) => {
    const { id, ...recipeWithoutId } = unitFixture.recipeDTOMock()
    expect(() => {
      Recipe.assertCanCreate(recipeWithoutId as any)
    }).toThrow(InvalidPostIdError)
    expect(Recipe.canCreate(recipeWithoutId as any)).toBe(false)
  })

  it('does not allow creating a recipe with an id with empty spaces', ({ unitFixture }) => {
    expect(() => {
      Recipe.assertCanCreate({ ...unitFixture.recipeDTOMock(), id: ' te st ' })
    }).toThrow(InvalidPostIdError)
    expect(Recipe.canCreate({ ...unitFixture.recipeDTOMock(), id: ' te st ' })).toBe(false)
  })

  it('can reconstruct a recipe from props', ({ unitFixture }) => {
    const props = unitFixture.recipeDTOMock()
    const recipe = Recipe.reconstruct({
      id: new PostId(props.id),
      createdById: new PostCreatorId(props.createdById),
      title: new PostTitle(props.title),
      visibility: new PostVisibility(props.visibility),
      description: new RecipeDescription(props.description),
      ingredients: props.ingredients,
      instructions: props.instructions,
      createdAt: new CreatedAtTimestamp(props.createdAt),
      updatedAt: new UnixTimestamp(props.updatedAt)
    })
    expect(recipe.id.value).toBe(props.id)
    expect(recipe.title.value).toBe(props.title)
    expect(recipe.visibility.value).toBe(props.visibility)
    expect(recipe.description.value).toBe(props.description)
    expect(recipe.ingredients).toStrictEqual(props.ingredients)
    expect(recipe.instructions).toStrictEqual(props.instructions)
  })

  it('canCreate returns true for valid props', ({ unitFixture }) => {
    const props = unitFixture.recipeDTOMock()
    expect(Recipe.canCreate(props)).toBe(true)
  })

  it('canCreate returns false for invalid props', ({ unitFixture }) => {
    const { id, ...invalidProps } = unitFixture.recipeDTOMock()
    expect(Recipe.canCreate(invalidProps as any)).toBe(false)
  })

  it('can compare recipes', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    const otherRecipe = unitFixture.recipeMock({ id: new PostId(uuid()) })
    expect(recipe.equals(recipe)).toBe(true)
    expect(recipe.equals(otherRecipe)).toBe(false)
  })

  it('updates description, ingredients, and instructions', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    recipe.updateDescription(new RecipeDescription('new description'), recipe.id)
    recipe.updateIngredients(['a', 'b'], recipe.id)
    recipe.updateInstructions(['step 1'], recipe.id)

    expect(recipe.description.value).toBe('new description')
    expect(recipe.ingredients).toEqual(['a', 'b'])
    expect(recipe.instructions).toEqual(['step 1'])
  })

  it('serializes and clones recipe state', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    const serialized = recipe.serialize()
    const clone = recipe.clone()

    expect(serialized.id).toBe(recipe.id.value)
    expect(clone.equals(recipe)).toBe(true)
  })

  it('delete updates timestamp', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    const before = recipe.updatedAt.value
    recipe.delete(recipe.id)
    expect(recipe.updatedAt.value).toBeGreaterThanOrEqual(before)
  })

  it('assertCanCreate rejects over-limit description', ({ unitFixture }) => {
    expect(() =>
      Recipe.assertCanCreate({
        ...unitFixture.recipeDTOMock(),
        description: 'x'.repeat(InputLimits.recipeDescription + 1)
      })
    ).toThrow(RecipeDescriptionTooLongError)
  })

  it('updateDescription rejects over-limit description', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    expect(() =>
      recipe.updateDescription(new RecipeDescription('x'.repeat(InputLimits.recipeDescription + 1)), recipe.id)
    ).toThrow(RecipeDescriptionTooLongError)
  })

  it('assertCanCreate rejects over-limit title', ({ unitFixture }) => {
    expect(() =>
      Recipe.assertCanCreate({
        ...unitFixture.recipeDTOMock(),
        title: 'x'.repeat(InputLimits.postTitle + 1)
      })
    ).toThrow(PostTitleTooLongError)
  })

  it('rename rejects over-limit title', ({ unitFixture }) => {
    const recipe = unitFixture.recipeMock()
    expect(() => recipe.rename(new PostTitle('x'.repeat(InputLimits.postTitle + 1)), recipe.id)).toThrow(PostTitleTooLongError)
  })
})

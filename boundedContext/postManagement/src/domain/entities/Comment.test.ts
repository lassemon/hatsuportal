...
describe('Comment', () => {
  ...
  it('can reconstruct a comment from props', ({ unitFixture }) => {
    ...
    const comment = Comment.reconstruct({
      ...
      body: props.body ? new CommentBody(props.body) : null,
  ...
  it('emits CommentUpdatedEvent on writeBody', ({ unitFixture }) => {
    const comment = unitFixture.commentMock()
    comment.clearEvents()
    comment.writeBody(new CommentBody('Updated body'))
  ...
  it('delete is idempotent when comment is already soft-deleted', ({ unitFixture }) => {
    const comment = unitFixture.commentMock({ isDeleted: true, body: null })
    comment.clearEvents()
    comment.delete(new UniqueId(unitFixture.sampleUserId))
    expect(comment.domainEvents).toHaveLength(0)
  })
  ...
  it('writeBody rejects over-limit body', ({ unitFixture }) => {
    const comment = unitFixture.commentMock()
    expect(() => comment.writeBody(new CommentBody('x'.repeat(InputLimits.commentBody + 1)))).toThrow(CommentBodyTooLongError)
  })
})

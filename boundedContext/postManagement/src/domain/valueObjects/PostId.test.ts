...
describe('PostId', () => {
  ...
  it('canCreate reflects validity', () => {
    expect(PostId.canCreate(uuid())).toBe(true)
    expect(PostId.canCreate('')).toBe(false)
  })

  it('assertCanCreate validates without returning a value', () => {
    const id = uuid()
    expect(() => PostId.assertCanCreate(id)).not.toThrow()
    expect(() => PostId.assertCanCreate('')).toThrow(InvalidPostIdError)
  })
})

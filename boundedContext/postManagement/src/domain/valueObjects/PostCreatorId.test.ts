...
describe('PostCreatorId', () => {
  ...
  it('canCreate reflects validity', () => {
    expect(PostCreatorId.canCreate(uuid())).toBe(true)
    expect(PostCreatorId.canCreate('')).toBe(false)
  })

  it('assertCanCreate validates without returning a value', () => {
    const id = uuid()
    expect(() => PostCreatorId.assertCanCreate(id)).not.toThrow()
    expect(() => PostCreatorId.assertCanCreate('')).toThrow(InvalidPostCreatorIdError)
  })

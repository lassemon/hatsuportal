...
describe('ImageLoadResult', () => {
  ...
  const buildValidImageProps = (): ImageAttachmentReadModelDTO => ({
    id: VALID_IMAGE_ID,
    storageKey: 'test.png',
    mimeType: 'image/png',
    size: 12345,
  ...
  const buildValidImage = () => buildValidImageProps()
  ...
  it('should create a failed result and return the error', () => {
    ...
    const result = ImageLoadResult.failedToLoad(VALID_IMAGE_ID, error.error)
  ...
  it('should match on success', () => {
    ...
    expect(matched).toBe(`success:${image.id}`)
  })
  ...
  it('tap should not call function on failure or notSet', () => {
    ...
    const failResult = ImageLoadResult.failedToLoad(VALID_IMAGE_ID, error.error)
    let called = false
    failResult.tap(() => {
      called = true
    })
  ...
  it('equals should return false for different success results', () => {
    const image = buildValidImage()
    const imageWithDifferentId = { ...buildValidImageProps(), id: 'different-id-98a976faödksjgf-akahsdk' }
    const a = ImageLoadResult.success(image)
  ...
  it('equals should return true for equal failed results', () => {
    ...
    const a = ImageLoadResult.failedToLoad(VALID_IMAGE_ID, error1.error)
    const b = ImageLoadResult.failedToLoad(VALID_IMAGE_ID, error2.error)
    expect(a.equals(b)).toBe(true)
  })
  ...
  it('toString should return a string representation', () => {
    ...
    const fail = ImageLoadResult.failedToLoad(VALID_IMAGE_ID, new Error('fail'))

...
export interface IStoryApplicationMapper {
  ...
  toDTOWithRelations(
    ...
    coverImageLoadResult: EntityLoadResult<ImageAttachmentReadModelDTO, ImageLoadError>,
...
export class StoryApplicationMapper implements IStoryApplicationMapper {
  ...
  dtoToDomainEntity(storyDTO: StoryDTO): Story {
    return Story.reconstruct({
      ...
      title: new NonEmptyString(storyDTO.title),
      body: new NonEmptyString(storyDTO.body),
      coverImageId: storyDTO.coverImageId ? new CoverImageId(storyDTO.coverImageId) : CoverImageId.NOT_SET,
      tagIds: storyDTO.tagIds.map((id) => new TagId(id)),
      createdAt: new CreatedAtTimestamp(storyDTO.createdAt),

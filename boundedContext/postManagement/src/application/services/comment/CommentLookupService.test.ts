...
describe('CommentLookupService', () => {
  ...
  it('delegates invalidateById to the read repository', ({ unitFixture }) => {
    const commentReadRepository = unitFixture.commentReadRepositoryMock()
    const service = new CommentLookupService(commentReadRepository, unitFixture.userGatewayMock())
    const commentId = new CommentId(unitFixture.sampleCommentId)

    service.invalidateById(commentId)

    expect(commentReadRepository.invalidateById).toHaveBeenCalledWith(commentId)
  })
})

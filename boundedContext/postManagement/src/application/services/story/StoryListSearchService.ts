...
import { omitNullAndUndefinedAndEmpty, OrderEnum, SortableKeyEnum, UserRoleEnum, VisibilityEnum } from '@hatsuportal/common'
...
export class StoryListSearchService implements IStoryListSearchService {
  ...
  async search({ loggedInUserId, searchCriteria }: StoryListSearchParams): Promise<{
    ...
    const loadUserResult = loggedInUserId ? await this.userGateway.getUserById({ userId: loggedInUserId }) : EntityLoadResult.skipped()
    ...
    if (loadUserResult.isSuccess() && _isEmpty(searchCriteria.visibility) && searchCriteria.onlyMyStories === false) {
      ...
      // covers both skipped() (anonymous user) and failure() (unrecognised userId)

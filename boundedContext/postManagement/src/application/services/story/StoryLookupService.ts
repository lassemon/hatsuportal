...
import { ApplicationError, EntityLoadResult, NotFoundError } from '@hatsuportal/platform'
import { OrderEnum, SortableKeyEnum, Logger, toHumanReadableJson } from '@hatsuportal/common'
...
export class StoryLookupService implements IStoryLookupService {
  ...
  private async loadImageSafely(imageId: string | null): Promise<EntityLoadResult<ImageAttachmentReadModelDTO, ImageLoadError>> {
    if (!imageId) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        return EntityLoadResult.failure(new ImageLoadError({ imageId, error }))
      } else {

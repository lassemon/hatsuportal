...
import { ApplicationError, EntityLoadError, EntityLoadResult, NotFoundError } from '@hatsuportal/platform'
import { OrderEnum, SortableKeyEnum, toHumanReadableJson } from '@hatsuportal/common'
...
export interface IStoryLookupService {
  ...
  findAll(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryWithRelationsDTO[]>
  findAllPublic(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryWithRelationsDTO[]>
  findById(id: PostId): Promise<StoryWithRelationsDTO | null>
  findByIds(ids: PostId[]): Promise<StoryWithRelationsDTO[]>
  findByImageId(imageId: PostId): Promise<StoryWithRelationsDTO[]>
  findAllVisibleForLoggedInCreator(creatorId: PostCreatorId, orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryWithRelationsDTO[]>
...
export class StoryLookupService implements IStoryLookupService {
  ...
  async findAll(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryWithRelationsDTO[]> {
    const stories = await this.storyRepository.findAll(orderBy, order)
    return this.enrichMany(stories)
  }

  async findAllPublic(orderBy: SortableKeyEnum, order: OrderEnum): Promise<StoryWithRelationsDTO[]> {
  ...
  invalidateById(id: PostId): void {
    this.storyRepository.invalidateById(id)
  }

  async findByIds(ids: PostId[]): Promise<StoryWithRelationsDTO[]> {
  ...
  private async enrichOne(baseStory: StoryReadModelDTO): Promise<StoryWithRelationsDTO> {
    ...
    if (!creatorLoadResult.isSuccess()) {
      throw new NotFoundError(`Creator not found for story ${baseStory.id}`)
    }
    ...
    let coverImageCreatedByName: string | null = creatorLoadResult.value.name

    if (coverImageResult.isSuccess()) {
      const imageCreatedById = coverImageResult.value.createdById
      const imageCreatorLoadResult = await this.userGateway.getUserById({ userId: imageCreatedById })
      if (!imageCreatorLoadResult.isSuccess()) {
        throw new NotFoundError(`Image creator not found for story ${baseStory.id}`)
      }
      coverImageCreatedByName = imageCreatorLoadResult.value.name
    }

    const creator = creatorLoadResult.value
    return this.storyApplicationMapper.toDTOWithRelations(
      baseStory,
      creator.name,
      coverImageResult,
      coverImageCreatedByName,
      tags,
      enrichedComments
    )
  }

  private async loadImageSafely(imageId: string | null): Promise<EntityLoadResult<ImageAttachmentReadModelDTO, EntityLoadError>> {
    if (!imageId) {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        return EntityLoadResult.failure(new EntityLoadError({ entityId: imageId, error }))
      } else {
  ...
  private async loadEnrichedCommentList(story: StoryReadModelDTO): Promise<CommentWithRelationsDTO[]> {
    ...
    return await Promise.all(
      commentListChunk.comments.map(async (comment: CommentWithRelationsDTO) => {
        ...
        if (!authorLoadResult.isSuccess()) {
          throw new NotFoundError(`Author not found for comment ${comment.id}`)
        }

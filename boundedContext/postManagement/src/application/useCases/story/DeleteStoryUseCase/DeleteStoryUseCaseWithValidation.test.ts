...
describe('DeleteStoryUseCaseWithValidation', () => {
  ...
  it('denies delete by non-author viewer via real ABAC', async ({ unitFixture }) => {
    const scenario = await DeletestoryValidationScenario.given()
      ...
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput({
        deletedById: unitFixture.sampleNonAuthorUserId,
        deleteStoryInput: {
          storyIdToDelete: unitFixture.storyDTOMock().id
        },
        storyDeleted: vi.fn(),
  ...
  it('allows delete by admin via real ABAC', async ({ unitFixture }) => {
    const scenario = await DeletestoryValidationScenario.given()
      .withAdminUser()

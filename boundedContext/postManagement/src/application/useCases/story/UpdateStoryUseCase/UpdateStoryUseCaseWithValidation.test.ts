...
import { AuthorizationError, NotFoundError, InvalidInputError, BASE64_PNG_PREFIX, AuthenticationError } from '@hatsuportal/platform'
...
describe('UpdateStoryUseCaseWithValidation', () => {
  ...
  it('should throw AuthenticationError when logged in user is not found', async ({ unitFixture }) => {
    const scenario = await UpdateStoryValidationScenario.given()
      .withoutLoggedInUser()
    ...
    scenario.thenUnderlyingUseCaseNotExecuted(scenario.useCaseMock).thenOutputBoundaryNotCalled('storyUpdated')
  })
  ...
  it('should throw InvalidInputError when updateStoryData.body is empty', async ({ unitFixture }) => {
    const scenario = await UpdateStoryValidationScenario.given()
      .withLoggedInUser()
      .withExistingStory()
      .expectErrorOfType(InvalidInputError)

...
describe('UpdateStoryUseCaseWithValidation', () => {
  ...
  it('denies update by non-author via real ABAC', async ({ unitFixture }) => {
    const scenario = await UpdateStoryValidationScenario.given()
      .withNonAuthorUser()
      .withExistingStory()
      ...
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput({
        updatedById: unitFixture.sampleNonAuthorUserId,
        updateStoryInput: {
          id: unitFixture.sampleStoryId,
          title: 'Updated',
          body: 'Updated desc',
  ...
  it('denies update by inactive user via real ABAC', async ({ unitFixture }) => {
    const scenario = await UpdateStoryValidationScenario.given()
      .withInactiveUser()
      .withExistingStory()
      .withActualAuthorizationService()

...
export class UpdateStoryUseCase implements IUpdateStoryUseCase {
    ...
    try {
      const [savedStory] = await this.transactionManager.execute<[Story]>(async () => {
        ...
        if (!isUndefined(updateStoryInput.image)) {
          ...
          const { imageIdForStory } = await this.syncCoverImage(updatedById, updatedStory!.id.value, currentImage, updateStoryInput.image)
  ...
  private async syncCoverImage(
  ...
  ): Promise<{ imageIdForStory: string | null }> {
    ...
    if (!currentImage) {
      ...
      return { imageIdForStory: createStagedImageResult.imageId }
    } else {

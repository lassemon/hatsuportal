...
import { CoverImageId, IStoryWriteRepository, PostId, PostTitle, PostVisibility, Story, StoryBody, TagId } from '../../../../domain'
...
export class UpdateStoryUseCase implements IUpdateStoryUseCase {
  ...
  async execute({ updatedById, updateStoryInput, storyUpdated, updateConflict }: IUpdateStoryUseCaseOptions) {
    ...
    const storyIdToUpdate = updateStoryInput.id
    ...
    try {
      const [savedStory] = await this.unitOfWork.execute<[Story]>(async () => {
        ...
        const existingStory = await this.storyRepository.findByIdForUpdate(new PostId(storyIdToUpdate))
        if (!existingStory) {
        ...
        if (!isUndefined(updateStoryInput.title)) updatedStory.rename(new PostTitle(updateStoryInput.title), updatedBy)
        ...
        if (!isUndefined(updateStoryInput.body)) updatedStory.updateBody(new StoryBody(updateStoryInput.body), updatedBy)

...
import { PostId, PostTitle, PostVisibility, StoryBody } from '../../../../domain'
...
export class UpdateStoryUseCaseWithValidation extends UseCaseWithValidation<IUpdateStoryUseCaseOptions> implements IUpdateStoryUseCase {
  ...
  async execute(options: IUpdateStoryUseCaseOptions): Promise<void> {
    ...
    if (userLoadResult.isFailed()) throw new AuthenticationError('Not logged in.')
  ...
  private validateDomainRules(options: IUpdateStoryUseCaseOptions): boolean {
    ...
    return (
      ...
      ((title ?? null) !== null ? this.testArgumentInstance(PostTitle, 'updateStoryInput.title', options) : true) &&
      ((body ?? null) !== null ? this.testArgumentInstance(StoryBody, 'updateStoryInput.body', options) : true) &&

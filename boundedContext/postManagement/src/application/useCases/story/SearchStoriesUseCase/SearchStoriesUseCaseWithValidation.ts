import { isBoolean, isEnumValue, isString, Logger, OrderEnum, SortableKeyEnum, VisibilityEnum } from '@hatsuportal/common'
import { AuthorizationError, EntityLoadResult, InvalidInputError, UseCaseWithValidation } from '@hatsuportal/platform'
  ...
  implements ISearchStoriesUseCase
{
  ...
  private async validateAuthorization(options: ISearchStoriesUseCaseOptions, loggedInUserId?: string): Promise<boolean> {
    ...
    const userLoadResult = loggedInUserId ? await this.userGateway.getUserById({ userId: loggedInUserId }) : EntityLoadResult.skipped()
    ...
    const authorizationResult = this.authorizationService.canSearchStories(
      userLoadResult.isSkipped() ? null : userLoadResult.value,
      searchCriteria
    )

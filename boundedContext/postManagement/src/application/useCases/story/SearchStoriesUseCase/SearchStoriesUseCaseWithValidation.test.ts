...
import { OrderEnum, SortableKeyEnum, VisibilityEnum, DeepPartial } from '@hatsuportal/common'
...
describe('SearchStoriesUseCaseWithValidation', () => {
  ...
  it('denies anonymous search with visibility filter via real ABAC', async () => {
    const scenario = await SearchStoriesValidationScenario.given()
  ...
  it('denies anonymous search with onlyMyStories via real ABAC', async () => {
    const scenario = await SearchStoriesValidationScenario.given()
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput(
  ...
  it('throws AuthorizationError when supplied user id fails to load', async ({ unitFixture }) => {
    const unknownUserId = 'unknown-user-a2f0-f95ccab82d92'
    const scenario = await SearchStoriesValidationScenario.given()
      .withFailedUserLoad(unknownUserId)
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput(baseInput(unknownUserId))

    scenario.thenUnderlyingUseCaseNotExecuted(scenario.useCaseMock)
  })

  it('allows anonymous search without restricted filters via real ABAC', async () => {
    const scenario = await SearchStoriesValidationScenario.given()
      .withActualAuthorizationService()
      .whenExecutedWithInput(baseInput(undefined))
  ...
  it('should throw InvalidInputError when orderBy filter contains invalid value', async ({ unitFixture }) => {
    const scenario = await SearchStoriesValidationScenario.given()
      ...
      .whenExecutedWithInput(
        baseInput(unitFixture.sampleUserId, {
          orderBy: 'invalid_order_by' as SortableKeyEnum
        })
      )

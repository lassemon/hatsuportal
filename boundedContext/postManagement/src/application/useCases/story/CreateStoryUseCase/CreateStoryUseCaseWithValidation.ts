import { UseCaseWithValidation, AuthorizationError, AuthenticationError } from '@hatsuportal/platform'
...
import { PostTitle, PostVisibility, StoryBody } from '../../../../domain'
...
export class CreateStoryUseCaseWithValidation extends UseCaseWithValidation<ICreateStoryUseCaseOptions> implements ICreateStoryUseCase {
  ...
  async execute(options: ICreateStoryUseCaseOptions): Promise<void> {
    ...
    if (loadResult.isFailed()) throw new AuthenticationError('Not logged in.')
  ...
  private validateDomainRules(options: ICreateStoryUseCaseOptions): boolean {
    ...
    this.testArgumentInstance(StoryBody, 'createStoryInput.body', options)
    this.testArgumentInstance(PostTitle, 'createStoryInput.title', options)

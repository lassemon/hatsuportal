...
import { NotFoundError, IUseCase, IUseCaseOptions, IUnitOfWork } from '@hatsuportal/platform'
...
import { IStoryWriteRepository, PostCreatorId, PostId, PostTitle, PostVisibility, Story, StoryBody, TagId } from '../../../../domain'
  ...
  async execute({ createdById, createStoryInput, storyCreated }: ICreateStoryUseCaseOptions) {
    ...
    const [savedStory] = await this.unitOfWork.execute<[Story]>(async () => {
      ...
      const story = Story.create({
        ...
        title: new PostTitle(createStoryInput.title),
        body: new StoryBody(createStoryInput.body),

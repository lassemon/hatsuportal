...
import { InvalidInputError, NotFoundError } from '@hatsuportal/platform'
...
describe('CreateStoryUseCase', () => {
  ...
  it('throws NotFoundError when story is missing from lookup after create', async ({ unitFixture }) => {
    const scenario = await CreateStoryScenario.given()
      .withLoggedInUser()
      .withoutExistingStory()
    ...
    scenario.thenOutputBoundaryNotCalled('storyCreated').thenWriteRepositoryCalledTimes('insert', 1)
  })
  ...
  it('throws InvalidInputError when tag resolution fails', async ({ unitFixture }) => {
    const scenario = await CreateStoryScenario.given()
      ...
      .whenExecutedWithInput(baseInput(unitFixture.sampleUserId, { tags: [{ id: 'missing-tag' }], image: null }))

    scenario.thenOutputBoundaryNotCalled('storyCreated').thenWriteRepositoryCalledTimes('insert', 0)
  })

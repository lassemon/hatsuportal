...
import { EntityTypeEnum, OrderEnum, SortableKeyEnum } from '@hatsuportal/common'
...
describe('SearchPostsUseCaseWithValidation', () => {
  ...
  it('authorizes as anonymous when supplied user id fails to load', async ({ unitFixture }) => {
    ...
    const scenario = await SearchPostsValidationScenario.given()
      ...
      .whenExecutedWithInput(baseInput(unknownUserId, { postType: EntityTypeEnum.Story }))

    scenario.thenUnderlyingUseCaseExecuted(scenario.useCaseMock)
  })

  it('skips story authorization for non-story post type search', async () => {
    const scenario = await SearchPostsValidationScenario.given().whenExecutedWithInput(
      baseInput(undefined, {

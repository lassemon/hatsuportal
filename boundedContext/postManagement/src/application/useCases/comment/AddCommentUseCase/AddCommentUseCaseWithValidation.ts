  ...
  private async validateAuthorization(loggedInUser: UserReadModelDTO, options: IAddCommentUseCaseOptions): Promise<boolean> {
    ...
    if (isReply(options.addCommentInput.target)) {
      ...
      if (!parentComment || parentComment.isDeleted) {
        throw new AuthorizationError(`Parent comment with id ${parentCommentId.value} does not exist.`)
      }
      if (!postId.equals(parentComment.postId)) {
        throw new AuthorizationError(
          `Trying to reply to parentCommentId '${parentCommentId.value}' but parent comment does not belong to the same postId '${postId.value}' that we are trying to reply to. Parent comment instead belongs to postId '${parentComment.postId.value}'.`
        )
      }

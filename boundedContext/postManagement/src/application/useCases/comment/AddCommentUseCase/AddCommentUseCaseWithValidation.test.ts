...
describe('AddCommentUseCaseWithValidation', () => {
  ...
  it('should throw NotFoundError if post does not exist', async ({ unitFixture }) => {
    const scenario = await AddCommentValidationScenario.given()
      .withLoggedInUser()
      .withoutExistingPost()
  ...
  it('denies reply when parent comment is deleted', async ({ unitFixture }) => {
    const scenario = await AddCommentValidationScenario.given()
      .withLoggedInUser()
      .withExistingPost()
      .withDeletedComment(unitFixture.commentMock({ id: new CommentId(unitFixture.sampleParentCommentId), isDeleted: true, body: null }))
      .expectErrorOfType(NotFoundError)
      .whenExecutedWithInput(replyInput(unitFixture))

...
describe('HardDeleteCommentUseCase', () => {
  ...
  it('should not call output boundary when domain event service fails after successful hard delete', async ({ unitFixture }) => {
    const scenario = await HardDeleteCommentScenario.given()
      .withExistingComment()
      .withCommentWithRelations()
    ...
    scenario.thenOutputBoundaryNotCalled('commentHardDeleted')
  })
  ...
  it('should hard delete an already soft-deleted comment without emitting a second CommentDeletedEvent', async ({ unitFixture }) => {
    const scenario = await HardDeleteCommentScenario.given()
      .withDeletedComment()
      .withCommentWithRelations()
      .whenExecutedWithInput(baseInput(unitFixture))
    ...
    scenario
      .thenOutputBoundaryCalled('commentHardDeleted', expect.any(Object))
      .thenWriteRepositoryCalledTimes('deletePermanently', 1)
      .thenDomainEventsNotPersisted([expect.any(CommentDeletedEvent)])
  })
})

...
import { EntityLoadResult, flattenDomainEventsFromHolders, IDomainEventService, IUnitOfWork } from '@hatsuportal/platform'
...
export { sampleUserId, sampleUserName, sampleStoryId, sampleRecipeId, sampleImageId, sampleTagId, sampleCommentId, sampleParentCommentId }
export const sampleNonAuthorUserId = 'b2c3d4e5-f6a7-4890-b123-456789abcdef'
...
export const storyPropsMock = (overrides: Partial<StoryProps> = {}): StoryProps => {
  return {
    ...
    body: overrides.body ? overrides.body : new StoryBody(storyDTOMock().body),
    ...
    title: overrides.title ? overrides.title : new PostTitle(storyDTOMock().title)
  }
}
...
export const recipeMock = (overrides: Partial<RecipeProps> = {}): Recipe => {
  return Recipe.reconstruct({
    ...
    title: overrides.title ? overrides.title : new PostTitle(recipeDTOMock().title),
    visibility: overrides.visibility ? overrides.visibility : new PostVisibility(recipeDTOMock().visibility),
    description: overrides.description ? overrides.description : new RecipeDescription(recipeDTOMock().description),
...
export const tagMock = (): Tag => {
  return Tag.reconstruct({
    ...
    name: new TagName(tagDTOMock().name),
...
export const postWriteRepositoryMock = (): Mocked<IPostWriteRepository> => {
...
export const storyReadRepositoryMock = (): Mocked<IStoryReadRepository> => {
...
export const storyWriteRepositoryMock = (): Mocked<IStoryWriteRepository> => {
...
export const commentPropsMock = (overrides: Partial<CommentProps> = {}): CommentProps => {
  ...
  return {
    ...
    body: overrides.body !== undefined ? overrides.body : dto.body ? new CommentBody(dto.body) : null,
...
export const commentWriteRepositoryMock = (): Mocked<ICommentWriteRepository> => {
...
export const commentReadRepositoryMock = (): Mocked<ICommentReadRepository> => {
...
export const postReadRepositoryMock = (): Mocked<IPostReadRepository> => {
...
export const tagRepositoryMock = (): Mocked<ITagRepository> => {
  class TagRepositoryMock implements ITagRepository {
    findById = vi.fn().mockResolvedValue(tagMock())
    findByIdForUpdate = vi.fn().mockResolvedValue(tagMock())

...
import { IDomainEvent, IDomainEventHolder } from '@hatsuportal/shared-kernel'
...
> {
  ...
  protected readonly transactionManager = Fixture.transactionManagerMock(this.domainEventService)
  ...
  withoutLoggedInUser() {
    this.userGateway.getUserById = vi.fn().mockResolvedValue(UserLoadResult.failedToLoad('test-user-id', new Error('User not found')))
    return this
  }
  ...
  // THEN helpers
  // ───────────────────────────────────────────────────────────────────
  thenDomainEventsPersisted(eventHolders: IDomainEventHolder<PostId, UnixTimestamp>[]) {
  ...
  thenDomainEventsNotPersisted(eventHolders: IDomainEventHolder<PostId, UnixTimestamp>[]) {
    this.ensureExecutionCompleted()
    this.ensureNoUnexpectedError()

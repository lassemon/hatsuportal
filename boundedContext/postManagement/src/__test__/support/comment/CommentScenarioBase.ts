...
> {
  ...
  withEmptyCommentList() {
    this.commentLookupService.listTopLevelForPost = vi.fn().mockResolvedValue({ comments: [], nextCursor: null })
    return this
  }

  withExhaustedCommentList() {
    this.commentLookupService.listTopLevelForPost = vi.fn().mockResolvedValue({
  ...
  withEmptyRepliesList() {
    this.commentLookupService.listReplies = vi.fn().mockResolvedValue({ replies: [], nextCursor: null })
    return this
  }

  withExhaustedRepliesList() {
    this.commentLookupService.listReplies = vi.fn().mockResolvedValue({
      replies: [Fixture.commentWithRelationsDTOMock()],
      nextCursor: null
    })
    return this
  }
  ...
  thenDomainEventServiceCalledTimes(times: number) {
    this.ensureExecutionCompleted()
    this.ensureNoUnexpectedError()
    expect(this.domainEventService.persistEvents).toHaveBeenCalledTimes(times)
    return this
  }

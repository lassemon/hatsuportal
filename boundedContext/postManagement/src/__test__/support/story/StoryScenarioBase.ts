...
export abstract class StoryScenarioBase<INPUT, CALLBACKS extends string> extends ScenarioBase<INPUT, CALLBACKS, typeof StoryDomainEvents> {
  ...
  withoutExistingImage() {
    const story = Fixture.storyMock().clone()
    story.updateCoverImage(CoverImageId.NOT_SET, story.createdById)
    story.clearEvents()
    this.storyWriteRepository.findByIdForUpdate = vi.fn().mockResolvedValue(story)
    this.storyReadRepository.findById = vi
      .fn()
      .mockResolvedValue(Fixture.storyReadModelDTOMock({ coverImageId: CoverImageId.NOT_SET.value }))
    this.storyLookupService.findById = vi.fn().mockResolvedValue(Fixture.storyReadModelDTOMock())
    this.mediaGateway.getImageById = vi
      .fn()
      .mockResolvedValue(EntityLoadResult.failure(new EntityLoadError({ entityId: 'test-image-id', error: new Error('Image not found') })))
    return this
  }
  ...
  mediaGatewayWillReject(method: keyof IMediaGateway, error: Error = new Error('Media gateway failure')) {
    // @ts-expect-error – the mock infra object definitely has this key
    this.mediaGateway[method] = vi.fn().mockRejectedValue(error)
    return this
  }

  resolveStoryTagIdsWillReject(error: Error = new Error('Tag resolution failure')) {
    this.resolveStoryTagIdsService.resolve = vi.fn().mockRejectedValue(error)
    return this
  }

  thenResolveStoryTagIdsCalledTimes(times: number) {
    this.ensureExecutionCompleted()
    this.ensureNoUnexpectedError()
    expect(this.resolveStoryTagIdsService.resolve).toHaveBeenCalledTimes(times)
    return this
  }

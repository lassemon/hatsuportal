...
describe('Image', () => {
  it('can create image with all properties', ({ unitFixture }) => {
    ...
    expect(image.storageFileName.value).toBe(`${unitFixture.imageDTOMock().createdById}_${unitFixture.imageDTOMock().fileName}`)
  ...
  it('sets file extension based on mime type, not filename string', ({ unitFixture }) => {
    ...
    expect(metadata.storageFileName.value).toBe(`${metadata.createdById.value}_test.gif.avi`)
  })
  ...
  it('creates image with proper base64 encoding prefix', ({ unitFixture }) => {
    const image = Image.create({ ...unitFixture.imageDTOMock(), base64: 'testbinarystring' })
  ...
  it('sets image base64 with proper encoding prefix', ({ unitFixture }) => {
    ...
    image.update({ base64: 'testbinarystring' })
    expect(image.base64.value).toBe('data:image/png;base64,testbinarystring')
  })
})

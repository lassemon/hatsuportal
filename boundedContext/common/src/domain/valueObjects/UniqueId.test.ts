...
describe('UniqueId', () => {
  ...
  describe('canCreate', () => {
    ...
    it('should throw if throwError is true and value is invalid', () => {
      expect(() => UniqueId.canCreate('', { throwError: true })).toThrow(InvalidUniqueIdError)
      expect(() => UniqueId.canCreate('not-a-uuid', { throwError: true })).toThrow(InvalidUniqueIdError)
    })
  })
})

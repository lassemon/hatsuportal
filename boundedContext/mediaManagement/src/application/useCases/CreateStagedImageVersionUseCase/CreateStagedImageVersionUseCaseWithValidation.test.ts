...
import { CreateStagedImageVersionValidationScenario } from '../../../__test__/support/image/CreateStagedImageVersionValidationScenario'
...
describe('CreateStagedImageVersionUseCaseWithValidation', () => {
  ...
  it('should allow Creator to create staged versions with real authorization', async ({ unitFixture }) => {
    const scenario = await CreateStagedImageVersionValidationScenario.given()
      .withCreatorUser()
      .withActualAuthorizationService()
      .whenExecutedWithInput(baseInput(unitFixture.sampleUserId, unitFixture.sampleImageId))
  ...
  it('should deny Viewer from creating staged versions with real authorization', async ({ unitFixture }) => {
    const scenario = await CreateStagedImageVersionValidationScenario.given()
      .withNonAdminUser()
      .withActualAuthorizationService()
  ...
  it('should deny inactive users from creating staged versions with real authorization', async ({ unitFixture }) => {
    const scenario = await CreateStagedImageVersionValidationScenario.given()
      .withInactiveUser()
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)

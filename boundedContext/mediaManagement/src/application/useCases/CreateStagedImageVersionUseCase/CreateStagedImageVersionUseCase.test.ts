...
import { CreateStagedImageVersionScenario } from '../../../__test__/support/image/CreateStagedImageVersionScenario'
...
describe('CreateStagedImageVersionUseCase', () => {
  ...
  it('should not call output boundary when repository fails', async ({ unitFixture }) => {
    const scenario = await CreateStagedImageVersionScenario.given()
      .persistenceServiceWillReject(['prepareStagedImageFile'], new DataPersistenceError('DB fail'))
  ...
  it('should not call output boundary when the unit of work fails', async ({ unitFixture }) => {
    const scenario = await CreateStagedImageVersionScenario.given()
      .transactionWillReject(new Error('Unit of work failure'))

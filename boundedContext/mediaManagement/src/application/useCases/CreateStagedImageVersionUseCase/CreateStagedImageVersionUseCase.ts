import { Base64Image, FileSize, ImageCreatorId, ImageId, ImageVersionId, MimeType, StagedImage } from '../../../domain'
...
export class CreateStagedImageVersionUseCase implements ICreateStagedImageVersionUseCase {
  ...
  public async execute({ createdById, createImageInput, imageCreated }: ICreateStagedImageVersionUseCaseOptions): Promise<void> {
    ...
    const stagedImageVersion = await this.imagePersistenceService.persistStaged(stagedImage)

    imageCreated(stagedImageVersion.imageId.value, stagedImageVersion.stagedVersionId.value)
  }
}

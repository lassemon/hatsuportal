...
import { PromoteImageVersionValidationScenario } from '../../../__test__/support/image/PromoteImageVersionValidationScenario'
...
describe('PromoteImageVersionUseCaseWithValidation', () => {
  ...
  it('should authorize cover-replace through repository and mapper without stubbing lookup service', async ({ unitFixture }) => {
    const scenario = await PromoteImageVersionValidationScenario.given()
      .withLoggedInUser()
  ...
  it('should deny non-author from promoting with real authorization', async ({ unitFixture }) => {
    const scenario = await PromoteImageVersionValidationScenario.given()
      .withNonAuthorUser()
      .withExistingImage()
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput(baseInput(unitFixture.sampleNonAuthorUserId, unitFixture.sampleImageId, unitFixture.sampleImageVersionId))

    scenario.thenUnderlyingUseCaseNotExecuted(scenario.useCaseMock)
  })

  it('should deny inactive author from promoting with real authorization', async ({ unitFixture }) => {
    const scenario = await PromoteImageVersionValidationScenario.given()
      .withInactiveUser()
      .withExistingImage()
      .withActualAuthorizationService()

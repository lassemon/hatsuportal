...
import { PromoteImageVersionScenario } from '../../../__test__/support/image/PromoteImageVersionScenario'
...
describe('PromoteImageVersionUseCase', () => {
  ...
  it('should not call output boundary when promotion fails', async ({ unitFixture }) => {
    const scenario = await PromoteImageVersionScenario.given()
      .withStagedImageOnly()
      .persistenceServiceWillReject(['promoteStagedVersion'], new DataPersistenceError('DB fail'))

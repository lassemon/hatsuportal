...
import { DeleteImageScenario } from '../../../__test__/support/image/DeleteImageScenario'
...
describe('DeleteImageUseCase', () => {
  ...
  it('should not call output boundary when repository fails', async ({ unitFixture }) => {
    const scenario = await DeleteImageScenario.given()
      .withExistingImage()
      .persistenceServiceWillReject(['deleteImageMetadata'], new DataPersistenceError('DB fail'))

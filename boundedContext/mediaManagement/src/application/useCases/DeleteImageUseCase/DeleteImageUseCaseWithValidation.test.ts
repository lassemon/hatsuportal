...
import { DeleteImageValidationScenario } from '../../../__test__/support/image/DeleteImageValidationScenario'
...
describe('DeleteImageUseCaseWithValidation', () => {
  ...
  it('should deny non-author from deleting with real authorization', async ({ unitFixture }) => {
    const scenario = await DeleteImageValidationScenario.given()
      .withNonAuthorUser()
      .withExistingImage()
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput(baseInput(unitFixture.sampleNonAuthorUserId, unitFixture.sampleImageId))

    scenario.thenUnderlyingUseCaseNotExecuted(scenario.useCaseMock)
  })

  it('should deny inactive author from deleting with real authorization', async ({ unitFixture }) => {
    const scenario = await DeleteImageValidationScenario.given()
      .withInactiveUser()
      .withExistingImage()
      .withActualAuthorizationService()

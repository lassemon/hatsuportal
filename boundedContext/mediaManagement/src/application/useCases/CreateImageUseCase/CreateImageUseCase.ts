...
import { CurrentImage, Base64Image, FileSize, ImageCreatorId, ImageId, ImageVersionId, MimeType } from '../../../domain'
...
export class CreateImageUseCase implements ICreateImageUseCase {
  ...
  async execute({ createdById, createImageInput, imageCreated }: ICreateImageUseCaseOptions): Promise<void> {
    ...
    const savedImage = await this.imagePersistenceService.persistCurrent(image)

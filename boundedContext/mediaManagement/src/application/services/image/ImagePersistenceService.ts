...
export class ImagePersistenceService implements IImagePersistenceService {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (fileWritten) {
            await this.imageFileService.deleteImageFromFileSystem(stagedImage.storageKey)
          }
        } catch (rollbackError) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (fileWritten) {
            await this.imageFileService.deleteImageFromFileSystem(currentImage.storageKey)
          }
        } catch (rollbackError) {
  ...
  async discardStaged({ imageId, stagedVersionId }: StagedImageVersionIdentifier): Promise<void> {
    try {
      ...
      await this.imageFileService.deleteImageFromFileSystem(new NonEmptyString(discardedVersion.storageKey))
    } catch (error) {
  ...
  async update(currentImage: CurrentImage): Promise<Image> {
    try {
      ...
      await Promise.all(
        stagedStorageKeys.map((storageKey) => this.imageFileService.deleteImageFromFileSystem(new NonEmptyString(storageKey)))
      )
      ...
      await Promise.all(
        prunedStorageKeys.map((storageKey) => this.imageFileService.deleteImageFromFileSystem(new NonEmptyString(storageKey)))
      )
  ...
  async delete(image: Image): Promise<void> {
    try {
      ...
      await Promise.allSettled(
        storageKeysForImage.map((storageKey) => this.imageFileService.deleteImageFromFileSystem(new NonEmptyString(storageKey)))
      )
    } catch (error) {

...
describe('ImageLookupService', () => {
  ...
  it('propagates storage load failure when metadata exists', async () => {
    const { imageRepository, imageStorageService, service } = setup()
    const storageError = new Error('storage file missing')
    imageRepository.findById.mockResolvedValue(Fixture.imageMetadataDTO())
    imageStorageService.getImage.mockRejectedValue(storageError)

    await expect(service.findById(new ImageId(Fixture.sampleImageId))).rejects.toThrow(storageError)
  })
})

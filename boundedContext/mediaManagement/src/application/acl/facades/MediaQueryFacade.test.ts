...
describe('MediaQueryFacade', () => {
  ...
  it('propagates lookup rejection without wrapping as NotFoundError', async () => {
    const { imageLookupService, facade } = setup()
    const storageError = new Error('storage file missing')
    imageLookupService.findById.mockRejectedValue(storageError)

    await expect(facade.getImageById({ imageId: Fixture.sampleImageId })).rejects.toThrow(storageError)
  })
})

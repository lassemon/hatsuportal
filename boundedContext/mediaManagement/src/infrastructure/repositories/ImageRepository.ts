...
export class ImageRepository extends RepositoryBase implements IImageRepository, ITransactionAware {
  ...
  private async updateImage(currentImage: CurrentImage, buffer: Buffer): Promise<Image> {
    ...
    const stagedVersionKeys = await database
      .table<ImageVersionDatabaseSchema>(this.imageVersionsTable)
      .select('storageKey')
      .where({ imageId: currentImage.id.value, isStaged: true })
    ...
    try {
      await database.transaction(async (transaction) => {
        ...
        await transaction.table<ImageVersionDatabaseSchema>(this.imageVersionsTable).insert(imageVersionToInsert).onConflict('id').ignore()
      ...
      await Promise.allSettled(
        stagedVersionKeys.map((version) => this.imageFileService.deleteImageFromFileSystem(new NonEmptyString(version.storageKey)))
      )

      // Enforce version retention: keep only the newest N versions
      await this.pruneOldVersions(currentImage.id.value, this.versionRetentionCount)
    } catch (error) {
  ...
  private async pruneOldVersions(imageId: string, retainCount: number): Promise<void> {
    const database = await this.database()

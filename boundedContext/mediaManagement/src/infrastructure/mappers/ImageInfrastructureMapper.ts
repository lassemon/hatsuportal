...
import { ImageVersionWithBase64DTO, IncompleteImageError } from '../../application'
...
export interface IImageInfrastructureMapper {
  ...
  toDomainEntityWithVersions(imageMetadataRecord: ImageMetadataDatabaseSchema, versions: ImageVersionWithBase64DTO[]): Image
  /**
   * Only safe/efficient for the promotion-scoped partial aggregate from toImageForPromotion
   * (0–2 versions). A fully-hydrated aggregate would still be correct but wastefully touch
   * every historical version row.
   */
  toPromotionUpdate(image: Image): ImagePromotionUpdate
}
...
export class ImageInfrastructureMapper implements IImageInfrastructureMapper {
  ...
  toDTO(imageMetadataRecord: ImageMetadataDatabaseSchema): ImageMetadataDTO {
    if (!imageMetadataRecord.versionId) {
      throw new IncompleteImageError(imageMetadataRecord.id)
    }

    return {
      id: imageMetadataRecord.id,
      versionId: imageMetadataRecord.versionId,
      currentVersionPointer: imageMetadataRecord.currentVersionId,
  ...
  toDomainEntityWithVersions(imageRecord: ImageMetadataDatabaseSchema, versions: ImageVersionWithBase64DTO[]): Image {
  ...
  toPromotionUpdate(image: Image): ImagePromotionUpdate {
    const promotedVersion = image.getCurrentVersionOrThrow()
    const demotedVersions = [...image.versions.values()].filter((version) => !version.isCurrent)

    if (demotedVersions.length > 1) {
      throw new DataPersistenceError(
        `Cannot persist promotion for image '${image.id.value}': expected at most one demoted version in partial aggregate.`
      )
    }

    if (!promotedVersion.isCurrent || promotedVersion.isStaged) {
      throw new DataPersistenceError(
        `Cannot persist promotion for image '${image.id.value}': promoted version '${promotedVersion.id.value}' must be current and not staged.`
      )
    }

    if (!image.currentVersionId.equals(promotedVersion.id)) {
      throw new DataPersistenceError(
        `Cannot persist promotion for image '${image.id.value}': currentVersionId pointer does not match promoted version.`
      )
    }
    ...
    return {
      imageId: image.id.value,
      currentVersionId: image.currentVersionId.value,
      previousCurrentVersionId: demotedVersions[0]?.id.value ?? null,
      promotedVersion: {
        id: promotedVersion.id.value,
        storageKey: promotedVersion.storageKey.value,
        isCurrent: promotedVersion.isCurrent,
        isStaged: promotedVersion.isStaged
      }
    }
  }
}

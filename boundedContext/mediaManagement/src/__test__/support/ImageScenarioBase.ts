...
import { CurrentImage, Image, StagedImage, ImageCreatedEvent, ImageDeletedEvent, ImageUpdatedEvent } from '../../domain'
...
export abstract class ImageScenarioBase<INPUT, CALLBACKS extends string> extends ScenarioBase<INPUT, CALLBACKS, typeof ImageDomainEvents> {
  ...
  protected constructor(callbackNames: CALLBACKS[]) {
    ...
    this.imagePersistenceService.persistStaged = vi
      .fn()
      .mockImplementation((image: StagedImage) => ({ imageId: image.imageId, stagedVersionId: image.id }))
    this.imageRepository.update = vi.fn().mockImplementation((image: CurrentImage) => {
      ...
      imageMock.addDomainEvent(
        new ImageUpdatedEvent({
          id: imageMock.id.value,
          oldImageId: imageMock.currentVersionId?.value ?? null,
          newImageId: imageMock.currentVersionId!.value,
          updatedAt: imageMock.updatedAt.value,
          updatedById: imageMock.createdById.value
        })
      )
      return imageMock
    })
    this.imagePersistenceService.update = vi.fn().mockImplementation((image: CurrentImage) => {
      const imageMock = Fixture.imageMock({
        id: image.id,
        createdAt: image.createdAt,
        createdById: image.createdById,
    ...
    this.imagePersistenceService.discardStaged = vi.fn().mockImplementation((image: CurrentImage) => {})
    this.imageRepository.delete = vi.fn().mockImplementation((image: Image) => image)
    this.imagePersistenceService.delete = vi.fn().mockImplementation((image: Image) => image)
  }
  ...
  withExistingImage(image = Fixture.imageMock()) {
    ...
    this.imagePersistenceService.persistStaged = vi.fn().mockResolvedValue(image)
    this.imagePersistenceService.persistCurrent = vi.fn().mockResolvedValue(image)
    this.imagePersistenceService.update = vi.fn().mockResolvedValue(image)
    this.imagePersistenceService.discardStaged = vi.fn().mockResolvedValue(image)
    this.imagePersistenceService.delete = vi.fn().mockResolvedValue(image)
    return this
  }
  ...
  withoutExistingImage() {
    ...
    this.imagePersistenceService.persistStaged = vi.fn().mockResolvedValue(null)
    this.imagePersistenceService.persistCurrent = vi.fn().mockResolvedValue(null)
    this.imagePersistenceService.update = vi.fn().mockResolvedValue(null)
    this.imagePersistenceService.discardStaged = vi.fn().mockResolvedValue(null)
    this.imagePersistenceService.delete = vi.fn().mockResolvedValue(null)
    return this
  }

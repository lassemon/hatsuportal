...
> {
  ...
  withoutLoggedInUser() {
    this.userGateway.getUserById = vi.fn().mockResolvedValue(UserLoadResult.notSet())
    return this
  }
  ...
  withAdminUser(admin = Fixture.userReadModelDTOMock({ roles: [UserRoleEnum.Admin] })) {
    this.userGateway.getUserById = vi.fn().mockResolvedValue(UserLoadResult.success(admin))
    return this
  }

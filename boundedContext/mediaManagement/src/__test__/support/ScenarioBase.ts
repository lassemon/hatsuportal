...
import { IDomainEvent, IDomainEventHolder, UnixTimestamp } from '@hatsuportal/shared-kernel'
...
> {
  ...
  withAdminUser(admin = Fixture.userReadModelDTOMock({ roles: [UserRoleEnum.Admin] })) {
    this.userGateway.getUserById = vi.fn().mockResolvedValue(EntityLoadResult.success(admin))
    return this
  }
  ...
  withNonAuthorUser(
    user = Fixture.userReadModelDTOMock({ id: Fixture.sampleNonAuthorUserId, roles: [UserRoleEnum.Creator] })
  ) {
    this.userGateway.getUserById = vi.fn().mockResolvedValue(EntityLoadResult.success(user))
    return this
  }

  transactionWillReject(error = new Error('Unit of work failure')) {
  ...
  domainEventServiceWillReject(error = new Error('Domain event service failure')) {
    this.domainEventService.persistToOutbox = vi.fn().mockRejectedValue(error)
    return this
  }
  ...
  // THEN helpers
  // ───────────────────────────────────────────────────────────────────
  thenDomainEventsPersisted(eventHolders: IDomainEventHolder<ImageId, UnixTimestamp>[]) {
    this.ensureExecutionCompleted()
    this.ensureNoUnexpectedError()

    eventHolders.forEach((eventHolder) => expect(this.domainEventService.persistToOutbox).toHaveBeenCalledWith([eventHolder]))
    return this
  }

  thenDomainEventsNotPersisted(eventHolders: IDomainEventHolder<ImageId, UnixTimestamp>[]) {
    this.ensureExecutionCompleted()
    this.ensureNoUnexpectedError()
    eventHolders.forEach((eventHolder) => expect(this.domainEventService.persistToOutbox).not.toHaveBeenCalledWith([eventHolder]))
    return this
  }

...
import { ITransactionManager, ITransactionAware, IDomainEventService } from '@hatsuportal/platform'
...
export const imageMock = (overrides: Partial<ImageProps> = {}, versionOverrides: Partial<ImageVersionProps> = {}): Image => {
  ...
  return Image.reconstruct({
    ...
    createdAt: overrides.createdAt ? overrides.createdAt : new CreatedAtTimestamp(imageDTOMock().createdAt),
...
export const imageLookupServiceMock = (): Mocked<IImageLookupService> => {
  class ImageLookupServiceMock implements IImageLookupService {
    ...
    findByIdAndVersionId = vi.fn().mockResolvedValue(imageMock())
  }
...
export const imagePersistenceServiceMock = (): Mocked<IImagePersistenceService> => {
  class ImagePersistenceServiceMock implements IImagePersistenceService {
    persistStaged = vi.fn().mockResolvedValue(imageMock())
    persistCurrent = vi.fn().mockResolvedValue(imageMock())
    discardStaged = vi.fn().mockResolvedValue(imageMock())
...
export const imageRepositoryMock = (): Mocked<IImageRepository & ITransactionAware> => {
  class ImageRepositoryMock implements IImageRepository {
    ...
    insertStaged = vi.fn().mockResolvedValue(imageMock())
    insertCurrent = vi.fn().mockResolvedValue(imageMock())
    rollbackCurrentVersion = vi.fn()
    pruneOldVersions = vi.fn().mockResolvedValue([])
    update = vi.fn().mockResolvedValue(imageMock())
    discardStagedVersion = vi.fn().mockResolvedValue(imageMock())
    delete = vi.fn().mockResolvedValue([])
...
export const unitOfWorkMock = (domainEventServiceMock: Mocked<IDomainEventService>): ITransactionManager<ImageId, UnixTimestamp> => {
  class UnitOfWorkMock implements ITransactionManager<ImageId, UnixTimestamp> {

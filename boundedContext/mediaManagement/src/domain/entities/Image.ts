...
import { IDomainEvent, IDomainEventHolder } from '@hatsuportal/foundation'
...
export class Image extends Entity implements IDomainEventHolder<ImageId, UnixTimestamp> {
  static canCreate(props: any, { throwError = false }: CanCreateOptions = {}) {
    try {
      new Image(
        ...
        props.createdAt instanceof UnixTimestamp ? props.createdAt : new UnixTimestamp(props.createdById),
        props.createdById instanceof ImageCreatorId ? props.createdById : new ImageCreatorId(props.createdById),
        props.currentVersionId instanceof ImageVersionId ? props.currentVersionId : new ImageVersionId(props.currentVersionId),
        props.versions.map((version: any) => [version.id.value, ImageVersion.reconstruct(version)]),
  ...
  get fileExtension(): string | null {
    const mimeType = this.getCurrentVersion()?.mimeType
    if (!mimeType) {
      return null
    }
  ...
  private assertInvariants(): void {
    ...
    if (this.currentVersionId === null) {
      ...
      if (!pointed.isCurrent || pointed.isStaged) {
        throw new InvalidDomainOperationError(`Pointer must reference a current (non-staged) version`)
      }
    }
  }
...
export class ImageVersion {
  ...
  private constructor(
    ...
    const fileExtension = mime.extension(this.mimeType.value)
    if (!fileExtension) {
      throw new InvalidMimeTypeError(`Could not parse mime type from '${this.mimeType.value}'`)
    }
  }

import { EntityFactoryResult, Maybe, unixtimeNow } from '@hatsuportal/common'
 ...
 */
export class Image extends Entity implements IDomainEventHolder {
  ...
  promoteToCurrent(versionId: ImageVersionId, storageKey: ImageStorageKey): ImageVersion {
    const target = this.validatePromotionTarget(versionId, storageKey)
    if (target.isCurrent) {
      return target
    }
    ...
    target.promoteToCurrent(storageKey)
  ...
  private validatePromotionTarget(versionId: ImageVersionId, storageKey: ImageStorageKey): ImageVersion {
...
export class ImageVersion {
  ...
  promoteToCurrent(permanentKey: ImageStorageKey): void {
    this.isCurrent = true
    this.isStaged = false

...
/** Metadata for a staged image whose bytes were already uploaded to object storage. */
export interface PreparedStagedMetadataCommand {
  imageId: ImageId
  stagedVersionId: ImageVersionId
  storageKey: ImageStorageKey // staged === true
  mimeType: MimeType
...
export class StagedImage {
  ...
  /**
   * Registers staged image metadata after {@link prepareStagedImageFile} has uploaded bytes to storage.
   * Inline base64 is not required — content is referenced by {@link PreparedStagedMetadataCommand.storageKey}.
   */
  static fromPreparedMetadata(command: PreparedStagedMetadataCommand): StagedImage {
    if (!command.storageKey.staged) {
      throw new StagedStorageKeyMismatchError(
        `Cannot register staged image version '${command.stagedVersionId.value}' with non-staged storage key '${command.storageKey.value}'`
      )
    }
    ...
    const stagedVersion = ImageVersion.staged({
      ...
      mimeType: command.mimeType,
      size: command.size,
      base64: Base64Image.forExternalStorage(command.mimeType),
      storageKey: command.storageKey,
      isCurrent: false,
      isStaged: true,
      createdById: command.createdById,

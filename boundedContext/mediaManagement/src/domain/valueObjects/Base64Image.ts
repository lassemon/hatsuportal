...
export class Base64Image extends ValueObject<string> {
  ...
  public static create(dataUrl: string, options?: Base64ImageOptions): Base64Image {
    ...
    const canonicalMime = MimeType.fromString(parsed.mimeType).value
  ...
  /**
   * Placeholder for staged image metadata registration when bytes already live in object storage.
   * Not valid for resize, upload, or display — use {@link assertHasInlineContent} before reading bytes.
   * TODO: remove this method when we refactor base64 in image read paths, so that metadata can truly be saved separately
   * and frontend is delivered an url for the image instead of the full base64 string
   */
  public static forExternalStorage(mimeType: MimeType): Base64Image {
    const canonicalMime = MimeType.fromString(mimeType.value).value
    Base64Image.ensureMimeTypeIsValidImage(canonicalMime)

    const payload = 'AA=='
    const decodedBytes = Base64Image.decodeBase64(payload)

    return new Base64Image(`data:${canonicalMime};base64,${payload}`, canonicalMime, decodedBytes, true)
  }

  public isExternalStorageReference(): boolean {
    return this._isExternalStorageReference
  }

  public assertHasInlineContent(): void {
    if (this._isExternalStorageReference) {
      throw new InvalidBase64ImageError('Base64Image references external storage and has no inline image content.')
    }
  }
  ...
  private constructor(
    ...
    private readonly _isExternalStorageReference = false
  ) {
    super()
    this.value = dataUrl

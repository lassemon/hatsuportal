...
describe('UserApiMapper', () => {
  ...
  it('throws on create when authentication or required fields are missing', ({ unitFixture }) => {
    ...
    expect(() => userMapper.toCreateUserInputDTO({ ...createRequest, password: '' }, unitFixture.sampleUserId)).toThrow(InvalidRequestError)
  })
  ...
  it('throws on update when authentication is missing', ({ unitFixture }) => {
    expect(() => userMapper.toUpdateUserInputDTO(unitFixture.updateUserRequest(), unitFixture.sampleUserId)).toThrow(InvalidRequestError)
  })
  ...
  it('converts user dto to response', ({ unitFixture }) => {
    expect(userMapper.toResponse(unitFixture.userDTOMock())).toStrictEqual(omit(unitFixture.userDTOMock(), 'active'))
  })
})

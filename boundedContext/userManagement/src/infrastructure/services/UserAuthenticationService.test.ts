...
describe('UserAuthenticationService', () => {
  ...
  it('validates password change', async ({ unitFixture }) => {
    ...
    const userService = new UserAuthenticationService(userRepositoryMock, passwordFactory)
  ...
  it('rejects password change if user is not found', async ({ unitFixture }) => {
    ...
    const userService = new UserAuthenticationService(userRepositoryMock, passwordFactory)
    await expect(userService.validatePasswordChange(user.id, 'newPassword', 'passwordhash')).rejects.toThrow(AuthorizationError)
  })

...
describe('UserMapper', () => {
  ...
  it('converts user and password to insert query', async ({ unitFixture }) => {
    ...
    const insertQuery2 = await userMapper.toInsertQuery(unitFixture.userMock({ active: false }), new Password('TestPassword123'))
  ...
  it('converts user and password to update query', async ({ unitFixture }) => {
    ...
    const insertQuery2 = await userMapper.toUpdateQuery(unitFixture.userMock({ active: false }), new Password('TestPassword123'))
    expect(insertQuery2.active).toBe(false)
  })

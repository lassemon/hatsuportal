...
export class UpdateUserUseCase implements IUpdateUserUseCase {
  ...
  async execute({ updatedById, updateUserInput, userUpdated, updateConflict }: IUpdateUserUseCaseOptions): Promise<void> {
    try {
      ...
      if (!isUndefined(name)) updatedUser!.rename(new UserName(name), updatedById)
      if (!isUndefined(email)) updatedUser!.changeEmail(new Email(email), updatedById)
      if (!isUndefined(roles))
        updatedUser!.changeRoles(
          roles.map((role) => new UserRole(role)),
          updatedById
        )
      if (!isUndefined(active)) active ? updatedUser!.activate(updatedById) : updatedUser!.deactivate(updatedById)

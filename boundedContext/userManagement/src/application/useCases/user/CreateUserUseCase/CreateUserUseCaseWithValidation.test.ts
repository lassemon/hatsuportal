...
describe('CreateUserUseCaseWithValidation', () => {
  ...
  it('should allow superadmin to create admin user', async ({ unitFixture }) => {
    const scenario = await CreateUserValidationScenario.given()
      .withUserRoles(new UserRole(UserRoleEnum.SuperAdmin))
      .withActualAuthorizationService()
      .whenExecutedWithInput(baseInput(unitFixture.userDTOMock().id, { roles: [UserRoleEnum.Admin] }))

    scenario.thenUnderlyingUseCaseExecuted(scenario.useCaseMock)
  })

  it('should throw AuthorizationError when non-superadmin tries to create admin user', async ({ unitFixture }) => {
    const scenario = await CreateUserValidationScenario.given()
      .withUserRoles(new UserRole(UserRoleEnum.Admin))
      .withActualAuthorizationService()
      .expectErrorOfType(AuthorizationError)
      .whenExecutedWithInput(baseInput(unitFixture.userDTOMock().id, { roles: [UserRoleEnum.Admin] }))

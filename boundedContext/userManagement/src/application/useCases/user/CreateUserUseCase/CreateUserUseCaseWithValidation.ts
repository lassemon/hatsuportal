...
export class CreateUserUseCaseWithValidation extends UseCaseWithValidation<ICreateUserUseCaseOptions> implements ICreateUserUseCase {
  ...
  private validateAuthorization(loggedInUser: User, newUsersRoles: UserRoleEnum[]): boolean {
    const authorizationResult = this.authorizationService.canCreateUser(this.userMapper.toDTO(loggedInUser), newUsersRoles)

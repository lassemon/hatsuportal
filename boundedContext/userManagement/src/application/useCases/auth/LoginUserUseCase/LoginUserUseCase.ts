...
import { IUserWriteRepository, UserName } from '../../../../domain'
...
export class LoginUserUseCase implements ILoginUserUseCase {
  ...
  async execute({ loginUserInput, loginSuccess }: ILoginUserUseCaseOptions) {
    ...
    const userCredentials = await this.userWriteRepository.getUserCredentialsByUsername(new UserName(username))
    const user = await this.userWriteRepository.findByName(new UserName(username))

    if (!userCredentials || !user || !(await Encryption.compare(password, userCredentials.passwordHash)) || !user.active) {
      throw new AuthenticationError('Incorrect username or password')
    } else {
      const userDTO = this.userApplicationMapper.toDTO(user)
      const authToken = this.tokenService.createAuthToken(userDTO)
      const refreshToken = this.tokenService.createRefreshToken(userDTO)

      loginSuccess(authToken, refreshToken, userDTO)
    }
  }
}

import { defineRule, IAuthorizationRequest, Rule, UserRoleEnum } from '@hatsuportal/foundation'
...
const updateUserRule = defineRule<UpdateUserAuthorizationAttributes>()({
  ...
  condition: (request) => {
    ...
    if (!isSelf(request) && !isAdmin(request)) {
      return { allowed: false, reason: `User ${request.requester?.name} does not have proper role to update user ${request.resource?.id}.` }
    }
    return { allowed: true }
  }
})

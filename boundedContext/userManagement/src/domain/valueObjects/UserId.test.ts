...
describe('UserId', () => {
  ...
  it('exposes canCreate and assertCanCreate helpers', () => {
    const id = uuid()
    expect(UserId.canCreate(id)).toBe(true)
    expect(() => UserId.assertCanCreate(id)).not.toThrow()
    expect(UserId.canCreate('')).toBe(false)
    expect(() => UserId.assertCanCreate('')).toThrow(InvalidUserIdError)
  })
})

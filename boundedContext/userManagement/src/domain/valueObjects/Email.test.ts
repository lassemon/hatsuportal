...
describe('Email', () => {
  ...
  it('exposes canCreate and assertCanCreate helpers', () => {
    expect(Email.canCreate('test@testemail.com')).toBe(true)
    expect(() => Email.assertCanCreate('test@testemail.com')).not.toThrow()
    expect(Email.canCreate('plainaddress')).toBe(false)
  })
})

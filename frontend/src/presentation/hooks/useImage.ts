...
import { ImagePresentation } from '@hatsuportal/presentation-common'
import { StorageSyncError } from 'infrastructure/errors/StorageSyncError'
import { useUtilityServiceContext } from 'infrastructure'
import { IImageService } from 'application'

type UseImageReturn = [
  { loading: boolean; image: ImagePresentation | null | undefined; error?: any },
  (update: UpdateParam<ImagePresentation | null | undefined>) => void,
  () => void
]

const DEBUG = false

const useImage = (imageService: IImageService, imageId?: string | null): UseImageReturn => {
  ...
  const imageSetter = (update: UpdateParam<ImagePresentation | null | undefined>) => {

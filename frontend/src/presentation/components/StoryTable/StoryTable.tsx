...
import useImage from 'presentation/hooks/useImage'
import DeleteForeverIcon from '@mui/icons-material/DeleteForever'
import { yellow } from '@mui/material/colors'

import OpenInFullIcon from '@mui/icons-material/OpenInFull'
import CloseFullscreenIcon from '@mui/icons-material/CloseFullscreen'
import { StorySortableKeyEnum, OrderEnum, uuid, VisibilityEnum } from '@hatsuportal/common'
import { highlightTextPart } from 'application/utils/utils'
import { TablePaginationActions } from './TablePaginationActions'
import DeleteButton from 'presentation/components/DeleteButton'
import { localStorageColorModeAtom } from 'presentation/components/Theme/Theme'
import { StoryPresentation, SearchStoriesRequest } from '@hatsuportal/presentation'
import { authAtom, errorAtom, IAuthState, IImageService, IStoryService } from 'application'
...
interface StoryTableProps {
  ...
  imageApiService: IImageService
  setStoryList: (value: React.SetStateAction<StoryPresentation[]>) => void
  pageNumber: number
    ...
    <React.Fragment>
      ...
      >
        ...
        <TableCell onClick={() => setOpen(!open)} sx={{ textAlign: 'center', width: '0%' }}>
          {authState.user?.id === localStory.createdBy ? (
            <strong>{localStory.getCreatedByUserName(authState.user?.id)}</strong>
          ) : (
            <span>{localStory.createdByUserName}</span>
          )}
        </TableCell>
        {authState.loggedIn && (
          <TableCell onClick={() => setOpen(!open)} sx={{ textAlign: 'center', width: '0%' }}>
        ...
        {authState.loggedIn && (
          <TableCell
            ...
            onClick={
              localStory?.id === STORY_DEFAULTS.NEW_STORY_ID || localStory.createdBy !== authState.user?.id
                ? () => setOpen(!open)
                : undefined
            }
          >
            <Tooltip
              title={localStory?.id === STORY_DEFAULTS.NEW_STORY_ID || localStory.createdBy !== authState.user?.id ? '' : 'Delete story'}
              placement="top-end"
            >
              <div>
                <DeleteButton
                  ...
                  disabled={localStory?.id === STORY_DEFAULTS.NEW_STORY_ID || localStory.createdBy !== authState.user?.id}
                />
              </div>
            </Tooltip>
            <Dialog
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            ...
            >
              {loadingStory ? (
                ...
                <>
                  <p style={{ whiteSpace: 'pre' }}>{JSON.stringify(localStory, null, 2)}</p>

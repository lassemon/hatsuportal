import { castToEnum, StorySortableKeyEnum, OrderEnum, validateAndCastEnum } from '@hatsuportal/common'
import StoryTable from 'presentation/components/Story/StoryTable'
import StoryTableFilters from 'presentation/components/Story/StoryTable/StoryTableFilters'
...
import StoryGrid from 'presentation/components/Story/StoryGrid'
import { StoryPresentation, SearchStoriesRequest } from '@hatsuportal/presentation'
import { authAtom, errorAtom } from 'application'
        ...
        <span>
          <IconButton
            sx={{
              ...
              color: (theme) => (listViewMode === ListViewMode.GRID ? theme.palette.secondary.main : theme.palette.grey[500]),
              '&:hover': {
        ...
        <span>
          <IconButton
            sx={{
              ...
              color: (theme) => (listViewMode === ListViewMode.TABLE ? theme.palette.secondary.main : theme.palette.grey[500]),
              '&:hover': {
                background: (theme) => theme.palette.background.paper,
                color: (theme) => theme.palette.primary.light
              }
...
const AllStoriesPage: React.FC = () => {
  ...
  const [storyList, setStoryList] = useState<StoryPresentation[]>([])
    ...
    <Box sx={{ margin: '2em' }}>
      ...
      {listViewMode === ListViewMode.TABLE ? (
        <StoryTable
          ...
          stories={storyList}
          imageApiService={dataServiceContext.imageService}

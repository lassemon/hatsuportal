...
export const StoryEdit: React.FC<StoryEditProps> = ({ story, setStory, onUpdate, onClose, savingStory, deleteStory }) => {
  ...
  const onSubmit = (values: FormInputs) => {
    ...
    ;(Object.keys(dirtyFields) as Array<keyof FormInputs>).forEach((key) => {
      ...
      if (key === 'tags') {
        // Map form tags to API format: TagInputRequest is either { id } (existing) or { name } (new).
        // Heuristic: tag.id === tag.slug means "new tag" because:
        // - New tags come from normaliseTag() when user types a string → id is set to slug (e.g. id="react", slug="react")
        // - Existing tags come from API/story → id is UUID (e.g. id="421a58d0-...", slug="react")
        partial.tags = values.tags.map((tag) => {
          const isNewTag = tag.id === tag.slug
          return isNewTag
            ? { name: tag.label } // Backend will create the tag
            : { id: tag.id } // Reference existing tag by id
        })
      }
    })

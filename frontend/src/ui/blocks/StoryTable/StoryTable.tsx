...
import { HighlightText } from 'ui/shared/ui/HighlightText'
...
import { localStorageColorModeAtom } from 'ui/shared/state/localStorageColorModeAtom'
...
import { TablePaginationActions } from 'ui/shared/ui/TablePaginationActions'
...
import { OrderEnum, SortableKeyEnum, uuid, VisibilityEnum } from '@hatsuportal/common'
...
interface StoryTableProps {
  ...
  order: OrderEnum
  orderBy: string
  onRequestSort: (event: React.MouseEvent<unknown>, property: SortableKeyEnum) => void
  totalCount: number
  loading: boolean
}
...
export const StoryTable: React.FC<StoryTableProps> = ({
  ...
  const createSortHandler = (property: SortableKeyEnum) => (event: React.MouseEvent<unknown>) => {
      ...
      >
        ...
        >
          ...
          >
            <TableRow>
              <TableCell />
              <TableCell sortDirection={orderBy === SortableKeyEnum.TITLE ? order : false}>
                <TableSortLabel
                  active={orderBy === SortableKeyEnum.TITLE}
                  direction={orderBy === SortableKeyEnum.TITLE ? reverseOrder(order) : OrderEnum.Descending}
                  color="info"
                  onClick={createSortHandler(SortableKeyEnum.TITLE)}
                  sx={{
                    '&.Mui-active': {
              ...
              <TableCell sx={{ textAlign: 'center' }}>
                <TableSortLabel
                  active={orderBy === SortableKeyEnum.CREATED_BY}
                  direction={orderBy === SortableKeyEnum.CREATED_BY ? reverseOrder(order) : 'desc'}
                  onClick={createSortHandler(SortableKeyEnum.CREATED_BY)}
                >
                  created by
                </TableSortLabel>
              </TableCell>
...
const TableStoryRow: React.FC<TableStoryRowProps> = ({
  ...
  const redirectToStory = () => {
    navigate([
      ...
      { href: `/story/${localStory.id}`, label: `"${localStory.title}"` }
    ])
  }
      ...
      storyApiService
        ...
        .then((deletedStory) => {
          setStoryList((_storyList) => {
            return filter(_storyList, (_story) => _story.id !== deletedStory.id)
          })
          setSuccess({ message: `Story "${deletedStory.title}" deleted succesfully!` })
        })
        .catch((error) => {
          setError(error)
        })
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        >
          <HighlightText text={localStory.title} highlight={search} />
        </TableCell>
            ...
            >
              ...
              <DialogContent>
                <Typography variant="body2" paragraph={false}>
                  Are you sure you want to delete <strong>{localStory?.title}</strong>
                </Typography>
              </DialogContent>

import { StoryTable, StoryTableFilters } from 'ui/blocks/StoryTable'
import StoryGrid from 'ui/blocks/StoryGrid'
...
import { castToEnum, OrderEnum, SortableKeyEnum, validateAndCastEnum } from '@hatsuportal/common'
  ...
  return omitBy(
    {
      ...
      visibility: !isEmpty(searchParams.getAll('visibility'))
...
const AllStoriesPage: React.FC = () => {
  ...
  const fetchAndSetStories = async (filters: SearchStoriesRequest) => {
    try {
      ...
      const storyList = await entityServiceContext.storyService.search(filters, { signal: controller.signal }).finally(() => {
  ...
  const handleRequestSort = (event: React.MouseEvent<unknown>, property: SortableKeyEnum) => {
  ...
  return (
    <Box sx={{ margin: '1em 1.5em' }}>
      ...
      {listViewMode === ListViewMode.TABLE ? (
        <StoryTable
          storyApiService={entityServiceContext.storyService}
          stories={storyList}

...
describe('ErrorHandlingUseCaseDecorator', () => {
  ...
  it('wraps non-Error throws as ApplicationError with unknown message', async () => {
    const inner = { execute: vi.fn().mockRejectedValue('boom') }
    const decorated = new ErrorHandlingUseCaseDecorator(inner)

    await expect(decorated.execute({} as any)).rejects.toMatchObject({

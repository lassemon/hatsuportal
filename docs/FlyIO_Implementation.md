...
- `dev-db/test-data-seed.sql` _(new file)_
...
**Step 1 — Create `dev-db/test-data-seed.sql`** (extract every INSERT/UPDATE from the old `init.sql`, no DDL):
...
volumes:
  ...
  - ./test-data-seed.sql:/test-data-seed.sql:ro
...
Part 1
  ...
  A5a — unify dev Docker (test-data-seed.sql + init-dev.sh, delete init.sql)      ← A5
